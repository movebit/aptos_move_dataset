...
module DiemFramework::DiemAccount {
    ...
    spec initialize {
        ...
        include CreateTreasuryComplianceAccountAbortsIf {
            auth_key_prefix: dummy_auth_key_prefix
        };
        aborts_if exists<AccountFreezing::FreezingBit>(@TreasuryCompliance) with errors::ALREADY_PUBLISHED;
    ...
    spec should_track_limits_for_account {
        ...
        aborts_if false;
        ensures result
            == spec_should_track_limits_for_account<Token>(payer, payee, is_withdrawal);
    }
    ...
    spec schema DepositAbortsIfRestricted<Token> {
        ...
        include spec_should_track_limits_for_account<Token>(payer, payee, false) ==>
            AccountLimits::UpdateDepositLimitsAbortsIf<Token> {
                addr: VASP::spec_parent_address(payee)
            };
        aborts_if spec_should_track_limits_for_account<Token>(payer, payee, false)
            && !AccountLimits::spec_update_deposit_limits<Token>(
                amount, VASP::spec_parent_address(payee)
    ...
    spec schema DepositEnsures<Token> {
        ...
        ensures event::spec_guid_eq(
            global<DiemAccount>(payee).received_events,
    ...
    public fun tiered_mint<Token>(
        ...
        let coin =
            DesignatedDealer::tiered_mint<Token>(
                tc_account,
                mint_amount,
    ...
    spec tiered_mint {
        ...
        modifies global<AccountLimits::Window<Token>>(
            VASP::spec_parent_address(designated_dealer_address)
        );
    ...
    spec schema TieredMintAbortsIf<Token> {
        ...
        mint_amount: u64;
        tier_index: u64;
        include DesignatedDealer::TieredMintAbortsIf<Token> {
            dd_addr: designated_dealer_address,
            amount: mint_amount
        };
        ...
        include DepositOverflowAbortsIf<Token> {
            payee: designated_dealer_address,
            amount: mint_amount
        };
    }
    ...
    spec schema TieredMintEnsures<Token> {
        ...
        let post post_dealer_balance = global<Balance<Token>>(designated_dealer_address).coin
            .value;
        ...
        ensures post_currency_info
            == update_field(
                currency_info, total_value, currency_info.total_value + mint_amount
            );
    ...
    spec schema TieredMintEmits<Token> {
        ...
        include DesignatedDealer::TieredMintEmits<Token> {
            dd_addr: designated_dealer_address,
            amount: mint_amount
        };
    }
    ...
    spec schema WithdrawFromBalanceAbortsIf<Token> {
        ...
        include spec_should_track_limits_for_account<Token>(payer, payee, true) ==>
    ...
    spec withdraw_from {
        ...
        ensures event::spec_guid_eq(
            global<DiemAccount>(payer).received_events,
    ...
    spec preburn {
        ...
        ensures event::spec_guid_eq(
            global<DiemAccount>(dd_addr).sent_events,
            old(global<DiemAccount>(dd_addr).sent_events)
        );
        ensures event::spec_guid_eq(
            global<DiemAccount>(dd_addr).received_events,
    ...
    public fun extract_withdraw_capability(sender: &signer): WithdrawCapability acquires DiemAccount {
    ...
    spec extract_withdraw_capability {
        ...
        ensures global<DiemAccount>(sender_addr)
            == update_field(
                old(global<DiemAccount>(sender_addr)),
                withdraw_capability,
                option::spec_none()
            );
    ...
    public fun restore_withdraw_capability(cap: WithdrawCapability) acquires DiemAccount {
    ...
    spec restore_withdraw_capability {
        ...
        ensures global<DiemAccount>(cap_addr)
            == update_field(
                old(global<DiemAccount>(cap_addr)),
                withdraw_capability,
                option::spec_some(cap)
            );
    ...
    spec pay_from {
        ...
        include PayFromWithoutDualAttestation<Token> { payer: cap.account_address };
    ...
    spec schema PayFromEnsures<Token> {
        ...
        amount: u64;
        ensures payer == payee ==>
            balance<Token>(payer) == old(balance<Token>(payer));
        ensures payer != payee ==>
            balance<Token>(payer) == old(balance<Token>(payer)) - amount;
        ensures payer != payee ==>
            balance<Token>(payee) == old(balance<Token>(payee)) + amount;
    }
    ...
    public fun rotate_authentication_key(
        cap: &KeyRotationCapability, new_authentication_key: vector<u8>
    ...
    fun add_currencies_for_account<Token>(
        new_account: &signer, add_all_currencies: bool
    ) {
        ...
        move_to(
            ...
            DiemAccount {
                ...
                withdraw_capability: option::some(
                    WithdrawCapability { account_address: new_account_addr }
                ),
    ...
    spec make_account {
        ...
        ensures post_account_ops_cap
            == update_field(
                account_ops_cap, creation_events, account_ops_cap.creation_events
            );
    ...
    fun create_authentication_key(
        account: &signer, auth_key_prefix: vector<u8>
    ): vector<u8> {
    ...
    fun create_diem_root_account(auth_key_prefix: vector<u8>) acquires AccountOperationsCapability {
        ...
        move_to(
            &dr_account,
            DiemWriteSetManager {
                upgrade_events: event::new_event_handle<Self::AdminTransactionEvent>(
                    &dr_account
                )
            }
        );
    ...
    spec schema CreateDiemRootAccountAbortsIf {
        ...
        aborts_if exists<AccountFreezing::FreezingBit>(@DiemRoot) with errors::ALREADY_PUBLISHED;
        include CreateAuthenticationKeyAbortsIf;
    }
    ...
    fun create_treasury_compliance_account(
        dr_account: &signer, auth_key_prefix: vector<u8>
    ) acquires AccountOperationsCapability {
        DiemTimestamp::assert_genesis();
    ...
    spec schema CreateTreasuryComplianceAccountAbortsIf {
        ...
        include Roles::GrantRole {
            ...
            role_id: Roles::TREASURY_COMPLIANCE_ROLE_ID
        };
        aborts_if exists<SlidingNonce::SlidingNonce>(@TreasuryCompliance) with errors::ALREADY_PUBLISHED;
        aborts_if VASPDomain::tc_domain_manager_exists() with errors::ALREADY_PUBLISHED;
    }
    ...
    public fun create_designated_dealer<CoinType>(
        ...
        DesignatedDealer::publish_designated_dealer_credential<CoinType>(
            &new_dd_account, creator_account, add_all_currencies
        );
    ...
    public fun create_parent_vasp_account<Token>(
        ...
        spec {
            ...
            assert Roles::spec_has_treasury_compliance_role_addr(
                signer::address_of(creator_account)
            );
        }
    }
    ...
    public fun delegated_key_rotation_capability(addr: address): bool acquires DiemAccount {
        assert!(exists_at(addr), errors::not_published(EACCOUNT));
    ...
    public fun delegated_withdraw_capability(addr: address): bool acquires DiemAccount {
        assert!(exists_at(addr), errors::not_published(EACCOUNT));
    ...
    spec schema ScriptPrologueAbortsIf<Token> {
        ...
        include DiemTransactionPublishingOption::AbortsIfNoTransactionPublishingOption;
        /// Covered: L74 (Match 8)
        aborts_if !DiemTransactionPublishingOption::spec_is_script_allowed(
            sender, script_hash
    ...
    fun writeset_prologue(
        ...
        assert!(
            Roles::has_diem_root_role(&sender),
            errors::invalid_argument(PROLOGUE_EINVALID_WRITESET_SENDER)
        );
    ...
    spec schema MultiAgentScriptPrologueAbortsIf<Token> {
        ...
        include PrologueCommonAbortsIf<Token> {
            transaction_sender,
            txn_public_key: txn_sender_public_key
        };
    }
    ...
    spec prologue_common {
        let transaction_sender = signer::address_of(sender);
        let max_transaction_fee = txn_gas_price * txn_max_gas_units;
        include PrologueCommonAbortsIf<Token> { transaction_sender, max_transaction_fee };
    }
    ...
    fun writeset_epilogue(
        dr_account: signer, txn_sequence_number: u64, should_trigger_reconfiguration: bool
    ...
    spec schema WritesetEpilogueAbortsIf {
        ...
        include EpilogueCommonAbortsIf<XUS> {
            account: dr_account,
            txn_gas_price: 0,
            txn_max_gas_units: 0,
            gas_units_remaining: 0
        ...
        include should_trigger_reconfiguration ==>
            Roles::AbortsIfNotDiemRoot { account: dr_account };
        include should_trigger_reconfiguration ==>
    ...
    spec schema CreateValidatorAccountEnsures {
        ...
        include Roles::GrantRole {
            addr: new_account_address,
            role_id: Roles::VALIDATOR_ROLE_ID
        };
        ensures exists_at(new_account_address);
    ...
    spec schema CreateValidatorOperatorAccountAbortsIf {
        ...
        // from `ValidatorConfig::publish`
        include DiemTimestamp::AbortsIfNotOperating;
        aborts_if ValidatorOperatorConfig::has_validator_operator_config(
            new_account_address
    ...
    spec module {
        ...
        apply PreserveWithdrawCapAbsence to * except make_account, create_*_account, restore_withdraw_capability, initialize;
    ...
    spec module {
        ...
        apply BalanceNotDecrease<Token> to *<Token> except withdraw_from, withdraw_from_balance, staple_xdx, unstaple_xdx, preburn, pay_from, pay_by_signers, epilogue_common, epilogue, failure_epilogue, success_epilogue;
    }
    ...
    spec module {
        ...
        /// resource struct `AccountOperationsCapability` is persistent
        invariant update old(exists<DiemWriteSetManager>(@DiemRoot)) ==>
            exists<DiemWriteSetManager>(@DiemRoot);
    }
    ...
    spec module {
        ...
        invariant<CoinType>[suspendable] forall addr: address where exists<Balance<CoinType>>(
            addr
        ): exists_at(addr);
