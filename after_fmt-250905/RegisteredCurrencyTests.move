...
module DiemFramework::RegisteredCurrencyTests {
    ...
    fun cannot_call_initialize_as_non_diem_root(
        dr: signer, tc: signer, alice: signer
    ) {
        Genesis::setup(&dr, &tc);
    ...
    #[test(dr = @DiemRoot, tc = @TreasuryCompliance)]
    #[expected_failure(abort_code = 7, location = RegisteredCurrencies)]
    fun cannot_add_currency_whose_currency_code_has_already_been_taken(
        dr: signer, tc: signer
    ) {
        Genesis::setup(&dr, &tc);
