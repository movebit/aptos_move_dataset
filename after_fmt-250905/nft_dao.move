...
module dao_platform::nft_dao {
    ...
    public fun get_proposal(proposal_id: u64, nft_dao: address): Proposal acquires Proposals {
        ...
        assert!(
            table::contains(proposals, proposal_id),
            error::not_found(EPRPOSAL_ID_NOT_EXIST)
        );
    ...
    public fun create_dao_and_get_dao_address(
        ...
        move_to(
            ...
            DAO {
                ...
                governance_token: GovernanceToken {
                    creator: voting_token_collection_creator,
    ...
    public entry fun create_proposal(
        ...
        assert!(
            fcnt == vector::length(&arg_names),
            error::invalid_argument(EPROPOSAL_ARG_COUNT_NOT_MATCH_FUNCTION_COUNT)
        );
        assert!(
            fcnt == vector::length(&arg_values),
            error::invalid_argument(EPROPOSAL_ARG_COUNT_NOT_MATCH_FUNCTION_COUNT)
        );
        assert!(
            fcnt == vector::length(&arg_types),
            error::invalid_argument(EPROPOSAL_ARG_COUNT_NOT_MATCH_FUNCTION_COUNT)
        );
        ...
        assert!(
            start_time_sec > now,
            error::invalid_argument(EPROPOSAL_START_TIME_SHOULD_BE_IN_FUTURE)
        );
    ...
    public entry fun vote(
        ...
        assert!(
            table::contains(&proposals.proposals, proposal_id),
            error::not_found(EPROPOSAL_NOT_FOUND)
        );
        let proposal = table::borrow(&proposals.proposals, proposal_id);
        let now = timestamp::now_seconds();
        assert!(
            now < proposal.start_time_sec + dao.voting_duration,
            error::invalid_argument(EPROPOSAL_ENDED)
        );
        assert!(
            now > proposal.start_time_sec,
            error::invalid_argument(EPROPOSAL_NOT_STARTED)
        );
        ...
        vector::enumerate_ref(
            ...
            |i, token_name| {
                ...
                assert!(
                    !bucket_table::contains(&stats.no_votes, &token_id),
    ...
    public entry fun resolve(
        ...
        assert!(
            now >= proposal.start_time_sec + dao.voting_duration,
            error::invalid_argument(EPROPOSAL_NOT_END)
        );
        // assert the proposal is unresolved yet
        assert!(
            proposal.resolution == PROPOSAL_PENDING,
            error::invalid_argument(EPROPOSAL_RESOLVED)
        );
    ...
    public entry fun admin_veto_proposal(
        ...
        let dao = borrow_global_mut<DAO>(nft_dao);
        assert!(
            dao.admin == signer::address_of(admin),
    ...
    public entry fun offer_admin(
        admin: &signer, dao: address, new_admin: address
    ) acquires DAO {
        assert!(
            exists<DAO>(dao),
            error::not_found(EDAO_NOT_EXIST)
        ...
        assert!(
            admin_addr == dao_config.admin,
            error::permission_denied(EINVALID_ADMIN_ACCOUNT)
        );

        assert!(
            option::is_none(&dao_config.pending_admin),
            error::invalid_argument(EADMIN_ALREADY_OFFERED)
        );
    ...
    public entry fun cancel_admin_offer(admin: &signer, dao: address) acquires DAO {
        ...
        // DAO offer exists
        assert!(
            option::is_some(&dao_config.pending_admin),
            error::invalid_argument(EADMIN_OFFER_NOT_EXIST)
        );
    ...
    public entry fun admin_change_dao_name(
        admin: &signer, dao: address, new_name: String
    ) acquires DAO {
        assert!(
            exists<DAO>(dao),
            error::not_found(EDAO_NOT_EXIST)
    ...
    public entry fun admin_change_dao_threshold(
        admin: &signer, dao: address, new_threshold: u64
    ) acquires DAO {
        assert!(
            exists<DAO>(dao),
            error::not_found(EDAO_NOT_EXIST)
    ...
    public entry fun admin_change_dao_voting_duration(
        admin: &signer, dao: address, new_duration: u64
    ) acquires DAO {
        assert!(
            exists<DAO>(dao),
            error::not_found(EDAO_NOT_EXIST)
    ...
    public entry fun admin_change_dao_min_voting_power(
        admin: &signer, dao: address, new_power: u64
    ) acquires DAO {
        assert!(
            exists<DAO>(dao),
            error::not_found(EDAO_NOT_EXIST)
        ...
        vector::enumerate_ref(
            ...
            |i, function_name| {
                ...
                if (function_name == &string::utf8(b"transfer_fund")) {
                    ...
                    let dst_addr = property_map::read_address(
                        args, &string::utf8(b"dst")
                    );
                    ...
                    assert!(
                        function_name == &string::utf8(b"no_op"),
                        error::invalid_argument(ENOT_SUPPROTED_FUNCTION)
                    );
                };
            }
        );
    ...
    fun resolve_internal(
        resolver: Option<address>, proposal_id: u64, nft_dao: address
    ) acquires DAO, Proposals, ProposalVotingStatistics {
        ...
        if (option::is_some(&resolver)) {
            ...
            assert!(
                *option::borrow(&resolver) == dao.admin,
                error::permission_denied(EINVALID_ADMIN_ACCOUNT)
            );
        ...
        assert!(
            exists<ProposalVotingStatistics>(nft_dao),
            error::not_found(EVOTING_STATISTICS_NOT_FOUND)
        );
        let proposal_stat =
            &mut borrow_global_mut<ProposalVotingStatistics>(nft_dao).proposals;
        ...
        nft_dao_events::emit_resolve_event(proposal_id, nft_dao, proposal.resolution)
    }
    ...
    fun assert_function_valid(function_name: String, map: &PropertyMap) {
        if (function_name == string::utf8(b"transfer_fund")) {
            ...
            assert!(
                property_map::length(map) == 0,
                error::invalid_argument(ENOT_SUPPROTED_FUNCTION)
            );
        } else if (function_name == string::utf8(b"offer_nft")) {
            assert!(
                property_map::length(map) == 5,
                error::invalid_argument(ENOT_SUPPROTED_FUNCTION)
            );
    ...
    #[test_only]
    public fun setup_voting_token_distribution(
        creator: &signer, voter: &signer
    ) {
    ...
    public fun test_e2e_scenario(
        aptos_framework: &signer, creator: &signer, voter: &signer
    ) acquires DAO, Proposals, ProposalVotingStatistics {
        ...
        assert!(
            coin::balance<AptosCoin>(signer::address_of(voter)) == 45,
            1
        );
    }

    #[test(aptos_framework = @0x1, admin = @0xdeaf, new_admin = @0xaf)]
    public fun test_dao_offer_and_claim(
    ...
    public fun test_transferring_ownership_to_zero_address(
    ...
    #[test(aptos_framework = @0x1, creator = @0xdeaf, voter = @0xaf)]
    public fun test_resolve_with_no_sufficient_votes(
        aptos_framework: &signer, creator: &signer, voter: &signer
    ) acquires DAO, Proposals, ProposalVotingStatistics {
        timestamp::set_time_has_started_for_testing(aptos_framework);
        account::create_account_for_test(@0x1);
    ...
    public fun test_resolve_earlier_than_ending_time(
        aptos_framework: &signer, creator: &signer, voter: &signer
    ) acquires DAO, Proposals, ProposalVotingStatistics {
        timestamp::set_time_has_started_for_testing(aptos_framework);
        account::create_account_for_test(@0x1);
    ...
    #[test(aptos_framework = @0x1, creator = @0xdeaf, voter = @0xaf)]
    public fun test_admin_execute_proposal(
        aptos_framework: &signer, creator: &signer, voter: &signer
    ) acquires DAO, Proposals, ProposalVotingStatistics {
        timestamp::set_time_has_started_for_testing(aptos_framework);
        account::create_account_for_test(@0x1);
    ...
    #[test(aptos_framework = @0x1, creator = @0xdeaf, voter = @0xaf)]
    public fun test_admin_veto_a_proposal(
        aptos_framework: &signer, creator: &signer, voter: &signer
    ) acquires DAO, Proposals, ProposalVotingStatistics {
        timestamp::set_time_has_started_for_testing(aptos_framework);
        account::create_account_for_test(@0x1);
    ...
    #[test(aptos_framework = @0x1, creator = @0xdeaf, voter = @0xaf)]
    public fun test_set_dao_config(
        aptos_framework: &signer, creator: &signer, voter: &signer
    ) acquires DAO {
        timestamp::set_time_has_started_for_testing(aptos_framework);
        account::create_account_for_test(@0x1);
    ...
    #[test(aptos_framework = @0x1, admin = @0xdeaf, new_admin = @0xaf)]
    public fun test_admin_create_proposal_without_token(
        aptos_framework: &signer, admin: &signer, new_admin: &signer
    ) acquires DAO, Proposals {
        // admin creates a dao
        timestamp::set_time_has_started_for_testing(aptos_framework);
    ...
    #[test(aptos_framework = @0x1, creator = @0xdeaf, voter = @0xaf)]
    public fun test_transfer_multiple_nfts(
        aptos_framework: &signer, creator: &signer, voter: &signer
    ) acquires DAO, Proposals, ProposalVotingStatistics {
        timestamp::set_time_has_started_for_testing(aptos_framework);
        account::create_account_for_test(@0x1);
