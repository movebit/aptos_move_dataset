module 0xCAFE::test {
    ...
    public entry fun object_vec(
        msg: String, objs: vector<Object<ModuleData>>
    ) acquires ModuleData {
        vector::for_each(
            objs,
            |o| {
    ...
    public entry fun pass_optional_fixedpoint32(
        o: Object<ModuleData>, x: Option<FixedPoint32>
    ) acquires ModuleData {
    ...
    public entry fun pass_optional_vector_fixedpoint64(
        o: Object<ModuleData>, x: Option<vector<FixedPoint64>>, i: u64
    ) acquires ModuleData {
        let addr = aptos_std::object::object_address(&o);
        let s;
        if (std::option::is_none(&x)) {
            ...
            s = convert(
                aptos_std::fixed_point64::get_raw_value(*std::vector::borrow(&x, i))
            );
        };
    ...
    public entry fun pass_vector_optional_object(
        o: vector<Option<Object<ModuleData>>>, s: String, i: u64
    ) acquires ModuleData {
        let o = std::vector::borrow_mut(&mut o, i);
        if (std::option::is_none(o)) { return }
        else {
    ...
    public entry fun ensure_no_fabrication(
        my_precious: Option<MyPrecious>
    ) {
    ...
    public entry fun ensure_vector_vector_u8(
        o: Object<ModuleData>, _: vector<vector<u8>>
    ) acquires ModuleData {
        let addr = aptos_std::object::object_address(&o);
        // guaranteed to exist
        borrow_global_mut<ModuleData>(addr).state = std::string::utf8(
            b"vector<vector<u8>>"
        );
    }
