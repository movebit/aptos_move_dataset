...
module DiemFramework::Roles {
    ...
    spec grant_diem_root_role {
        ...
        include GrantRole {
            addr: signer::address_of(dr_account),
            role_id: DIEM_ROOT_ROLE_ID
        };
    }

    /// Publishes treasury compliance role. Granted only in genesis.
    public(friend) fun grant_treasury_compliance_role(
        treasury_compliance_account: &signer, dr_account: &signer
    ) acquires RoleId {
        DiemTimestamp::assert_genesis();
    ...
    spec grant_treasury_compliance_role {
        ...
        include CoreAddresses::AbortsIfNotTreasuryCompliance {
            account: treasury_compliance_account
        };
        include AbortsIfNotDiemRoot { account: dr_account };
        include GrantRole {
            addr: signer::address_of(treasury_compliance_account),
            role_id: TREASURY_COMPLIANCE_ROLE_ID
        };
    }
    ...
    public(friend) fun new_designated_dealer_role(
        creating_account: &signer, new_account: &signer
    ) acquires RoleId {
    ...
    spec new_designated_dealer_role {
        ...
        include GrantRole {
            addr: signer::address_of(new_account),
    ...
    spec new_validator_operator_role {
        ...
        include GrantRole {
            addr: signer::address_of(new_account),
            role_id: VALIDATOR_OPERATOR_ROLE_ID
        };
    }
    ...
    fun grant_role(account: &signer, role_id: u64) {
        assert!(
            !exists<RoleId>(signer::address_of(account)),
            errors::already_published(EROLE_ID)
        );
    ...
    fun has_role(account: &signer, role_id: u64): bool acquires RoleId {
        let addr = signer::address_of(account);
        exists<RoleId>(addr) && borrow_global<RoleId>(addr).role_id == role_id
    }
    ...
    public fun assert_validator_operator(
    ...
    spec schema AbortsIfNotParentVaspOrChildVasp {
        ...
        aborts_if !exists<RoleId>(addr) with errors::NOT_PUBLISHED;
        let role_id = global<RoleId>(addr).role_id;
        aborts_if role_id != PARENT_VASP_ROLE_ID && role_id != CHILD_VASP_ROLE_ID with errors::REQUIRES_ROLE;
    }
