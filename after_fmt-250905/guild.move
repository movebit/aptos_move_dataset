...
module guild::guild {
    ...
    #[view]
    /// Returns the guild token address by name
    public fun member_token_address(
        guild_token: Object<GuildToken>, member_token_name: String
    ): address acquires GuildToken {
        let guild_token_addr = object::object_address(&guild_token);
        let member_collection_name =
            &borrow_global<GuildToken>(guild_token_addr).member_collection_name;
        token::create_token_address(
            &guild_token_addr, member_collection_name, &member_token_name
        )
    }
    ...
    public fun test_guild(
        admin: &signer, guild_master: &signer, user: address
    ) acquires GuildToken, MemberToken, Config, Config {
