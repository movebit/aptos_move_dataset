module 0x1::NFTGallery {
    ...
    public fun has_token<TokenType: copy + store + drop>(
        owner: address, token_id: &guid::ID
    ...
    public fun get_token_balance<TokenType: copy + store + drop>(
        owner: address, token_id: &guid::ID
    ...
    public fun get_token_supply<TokenType: copy + store + drop>(
        owner: address, token_id: &guid::ID
    ): u64 acquires NFTGallery {
        let gallery = &borrow_global<NFTGallery<TokenType>>(owner).gallery;
        let index_opt = index_of_token<TokenType>(gallery, token_id);
    ...
    public fun get_token_content_uri<TokenType: copy + store + drop>(
        owner: address, token_id: &guid::ID
    ): vector<u8> acquires NFTGallery {
        let gallery = &borrow_global<NFTGallery<TokenType>>(owner).gallery;
        let index_opt = index_of_token<TokenType>(gallery, token_id);
        assert!(option::is_some(&index_opt), EID_NOT_FOUND);
        let token = vector::borrow(gallery, option::extract(&mut index_opt));
    ...
    public fun get_token_metadata<TokenType: copy + store + drop>(
        owner: address, token_id: &guid::ID
    ): TokenType acquires NFTGallery {
        let gallery = &borrow_global<NFTGallery<TokenType>>(owner).gallery;
        let index_opt = index_of_token<TokenType>(gallery, token_id);
        assert!(option::is_some(&index_opt), EID_NOT_FOUND);
        let token = vector::borrow(gallery, option::extract(&mut index_opt));
    ...
    public fun get_token_parent_id<TokenType: copy + store + drop>(
        owner: address, token_id: &guid::ID
    ): Option<guid::ID> acquires NFTGallery {
        let gallery = &borrow_global<NFTGallery<TokenType>>(owner).gallery;
        let index_opt = index_of_token<TokenType>(gallery, token_id);
        assert!(option::is_some(&index_opt), EID_NOT_FOUND);
        let token = vector::borrow(gallery, option::extract(&mut index_opt));
    ...
    public fun transfer_token_between_galleries_impl<TokenType: copy + store + drop>(
        ...
        if (NFT::is_data_inlined(vector::borrow(gallery, from_token_idx))
            ...
            let split_out_token =
                NFT::split_out(vector::borrow_mut(gallery, from_token_idx), amount);
