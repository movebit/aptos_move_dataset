...
module rewards_pool::rewards_pool_tests {
    ...
    fun test_e2e(
        claimer_1: &signer, claimer_2: &signer, claimer_3: &signer
    ) {
        test_helpers::set_up();
        // Create a rewards pool with 1 native fungible asset and 1 coin.
        let asset_rewards_1 =
            test_helpers::create_fungible_asset_and_mint(claimer_1, b"test1", 1000);
        let asset_rewards_2 =
            test_helpers::create_fungible_asset_and_mint(claimer_1, b"test2", 2000);
    ...
    fun test_claim_with_rounding_down(
        claimer_1: &signer, claimer_2: &signer
    ) {
        test_helpers::set_up();
        // Create a rewards pool with 1 native fungible asset and 1 coin.
        let asset_rewards_1 =
    ...
    fun test_claim_zero_rewards(claimer: &signer) {
        test_helpers::set_up();
        let asset_rewards =
            test_helpers::create_fungible_asset_and_mint(claimer, b"test1", 1);
        let rewards_pool =
    ...
    fun test_cannot_add_reward_for_unsupported_tokens(claimer: &signer) {
        test_helpers::set_up();
        let asset_rewards =
            test_helpers::create_fungible_asset_and_mint(claimer, b"test1", 1000);
        let non_reward_assets =
            test_helpers::create_fungible_asset_and_mint(claimer, b"test2", 1000);
        let rewards_pool =
            rewards_pool::create(vector[fungible_asset::asset_metadata(&asset_rewards)]);
    ...
    fun verify_claimer_shares_percentage(
        ...
        let (shares, _) =
            rewards_pool::claimer_shares(
                signer::address_of(claimer_1), rewards_pool, epoch
            );
    ...
    fun increase_alocation(
        claimer: &signer, pool: Object<RewardsPool>, amount: u64
    ) {
        rewards_pool::increase_allocation(signer::address_of(claimer), pool, amount);
    }

    fun decrease_alocation(
        claimer: &signer, pool: Object<RewardsPool>, amount: u64
    ) {
    ...
    fun claim_and_verify_rewards(
        ...
        vector::zip(
            rewards,
            expected_amounts,
            |reward, expected_amount| {
                let reward_metadata = fungible_asset::asset_metadata(&reward);
                let claimable_amount =
                    if (simple_map::contains_key(&claimable_map, &reward_metadata)) {
