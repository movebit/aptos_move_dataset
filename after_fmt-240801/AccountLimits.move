...
module DiemFramework::AccountLimitsTests {
    ...
    fun grant_mutation_capability_after_genesis(
        dr: signer, tc: signer, vasp: signer
    ) {
        Genesis::setup(&dr, &tc);
    ...
    fun publish_window_non_diem_root(
        dr: signer, tc: signer, vasp: signer
    ) {
        setup(&dr, &tc, &vasp);
    ...
    fun publish_window_non_existent_limit_address(
        dr: signer, tc: signer, vasp: signer
    ) {
        setup(&dr, &tc, &vasp);
    ...
    fun publish_unrestricted_limits_for_testing_twice(
        dr: signer, tc: signer, vasp: signer
    ) {
        setup(&dr, &tc, &vasp);
        AccountLimits::publish_unrestricted_limits_for_testing<XUS>(&vasp);
    ...
    #[test(dr = @DiemRoot, tc = @TreasuryCompliance, vasp = @0x2)]
    fun update_limits_definition_twice(
        dr: signer, tc: signer, vasp: signer
    ) {
        setup(&dr, &tc, &vasp);
        AccountLimits::update_limits_definition<XUS>(
    ...
    fun update_limits_definition_non_tc(
        dr: signer, tc: signer, vasp: signer
    ) {
        setup(&dr, &tc, &vasp);
        AccountLimits::update_limits_definition<XUS>(
    ...
    #[test(dr = @DiemRoot, tc = @TreasuryCompliance, vasp = @0x2)]
    #[expected_failure(abort_code = 5, location = AccountLimits)]
    fun update_limits_definition_non_exsistent(
        dr: signer, tc: signer, vasp: signer
    ) {
        setup(&dr, &tc, &vasp);
        AccountLimits::update_limits_definition<XUS>(
