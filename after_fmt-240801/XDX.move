...
module DiemFramework::XDX {
    ...
    spec initialize {
        ...
        include AccountLimits::PublishUnrestrictedLimitsAbortsIf<XDX> {
        ...
        include AccountLimits::PublishUnrestrictedLimitsEnsures<XDX> {
            publish_account: dr_account
        };
    ...
    spec is_xdx {
        pragma opaque;
        include Diem::spec_is_currency<CoinType>() ==>
    ...
    spec module {
        ...
        invariant [suspendable] DiemTimestamp::is_operating() ==>
            Diem::spec_xdx_exchange_rate<XDX>()
                == fixed_point32::spec_create_from_rational(1, 1);
    ...
    spec module {
        ...
        invariant [suspendable] forall addr: address where exists<AccountLimits::LimitsDefinition<XDX>>(
            addr
        ): addr == @DiemRoot;

        /// `Reserve` is persistent
        invariant update old(reserve_exists()) ==> reserve_exists();
    }
}
