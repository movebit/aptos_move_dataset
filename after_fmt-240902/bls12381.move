...
module aptos_std::bls12381 {
    ...
    fun test_pubkey_validation_against_invalid_keys() {
        ...
        assert!(
            option::is_none(&public_key_from_bytes(maul_bytes(&pk_bytes))),
            1
        );
    }
    ...
    fun test_signature_aggregation() {
        ...
        assert!(
            option::is_none(&mut aggregate_signatures(vector[])),
            1
        );
    ...
    fun test_verify_pop_randomized() {
        ...
        assert!(
            option::is_none(
                &public_key_from_bytes_with_pop(
                    pk_bytes, &maul_proof_of_possession(&pop)
                )
            ),
            1
        );
        assert!(
            option::is_none(
                &public_key_from_bytes_with_pop(maul_bytes(&pk_bytes), &pop)
            ),
            1
        );
    }
