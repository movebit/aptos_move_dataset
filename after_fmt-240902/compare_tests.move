...
module std::compareTests {
    ...
    fun less_than_with_natural_ordering() {
        ...
        assert!(
            compare::cmp_bcs_bytes(&bcs::to_bytes(&false), &bcs::to_bytes(&true))
                == LESS_THAN,
            0
        );
        assert!(
        ...
        assert!(
            compare::cmp_bcs_bytes(&bcs::to_bytes(&0u256), &bcs::to_bytes(&1u256))
                == LESS_THAN,
            0
        );
    }
    ...
    fun less_than_without_natural_ordering() {
        ...
        assert!(
            compare::cmp_bcs_bytes(&bcs::to_bytes(&@0x01), &bcs::to_bytes(&@0x10))
                == LESS_THAN,
            0
        ); // sensible
        assert!(
            compare::cmp_bcs_bytes(&bcs::to_bytes(&@0x100), &bcs::to_bytes(&@0x001))
                == LESS_THAN,
            0
        ); // potentially confusing
        assert!(
            compare::cmp_bcs_bytes(&bcs::to_bytes(&x"00"), &bcs::to_bytes(&x"01"))
                == LESS_THAN,
            0
        ); // sensible
        assert!(
            compare::cmp_bcs_bytes(&bcs::to_bytes(&x"01"), &bcs::to_bytes(&x"10"))
                == LESS_THAN,
            0
        ); // sensible
        assert!(
            compare::cmp_bcs_bytes(&bcs::to_bytes(&x"0000"), &bcs::to_bytes(&x"01"))
                == LESS_THAN,
            0
        ); //
        assert!(
            compare::cmp_bcs_bytes(&bcs::to_bytes(&x"0100"), &bcs::to_bytes(&x"0001"))
                == LESS_THAN,
            0
        ); // potentially confusing
    }
    ...
    fun greater_than_with_natural_ordering() {
        ...
        assert!(
            compare::cmp_bcs_bytes(&bcs::to_bytes(&1u8), &bcs::to_bytes(&0u8))
                == GREATER_THAN,
            0
        );
        assert!(
    ...
    fun greater_than_without_natural_ordering() {
        ...
        assert!(
            compare::cmp_bcs_bytes(&bcs::to_bytes(&@0x10), &bcs::to_bytes(&@0x01))
                == GREATER_THAN,
            0
        ); // sensible
        assert!(
            compare::cmp_bcs_bytes(&bcs::to_bytes(&@0x001), &bcs::to_bytes(&@0x100))
                == GREATER_THAN,
            0
        ); // potentially confusing
        assert!(
        ...
        assert!(
            compare::cmp_bcs_bytes(&bcs::to_bytes(&x"10"), &bcs::to_bytes(&x"01"))
                == GREATER_THAN,
            0
        ); // sensible
        assert!(
            compare::cmp_bcs_bytes(&bcs::to_bytes(&x"01"), &bcs::to_bytes(&x"0000"))
                == GREATER_THAN,
            0
        ); // sensible
        assert!(
            compare::cmp_bcs_bytes(&bcs::to_bytes(&x"0001"), &bcs::to_bytes(&x"0100"))
                == GREATER_THAN,
            0
        ); // potentially confusing
    }
