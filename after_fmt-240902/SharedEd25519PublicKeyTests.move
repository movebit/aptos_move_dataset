...
module DiemFramework::SharedEd25519PublicKeyTests {
    ...
    fun publish_and_rotate_shared_key(
        dr: signer, tc: signer, account: signer
    ) {
        let dr = &dr;
        let tc = &tc;
        let account = &account;
    ...
    fun get_key_for_non_shared_account_should_fail(
        dr: signer, tc: signer, account: signer
    ) {
        setup(&dr, &tc, &account);
    ...
    fun publish_key_with_bad_length_1(
        dr: signer, tc: signer, account: signer
    ) {
        setup(&dr, &tc, &account);
    ...
    #[test(dr = @DiemRoot, tc = @TreasuryCompliance, account = @0x100)]
    #[expected_failure(abort_code = 7, location = SharedEd25519PublicKey)]
    fun publish_key_with_bad_length_2(
        dr: signer, tc: signer, account: signer
    ) {
        setup(&dr, &tc, &account);

        let invalid_pubkey =
            x"10003d4017c3e843895a92b70aa74d1b7ebc9c982ccf2ec4968cc0cd55f12af4660c";
        SharedEd25519PublicKey::publish(&account, invalid_pubkey);
    }

    #[test(dr = @DiemRoot, tc = @TreasuryCompliance, account = @0x100)]
    #[expected_failure(abort_code = 7, location = SharedEd25519PublicKey)]
    fun rotate_to_key_with_bad_length(
        dr: signer, tc: signer, account: signer
    ) {
        setup(&dr, &tc, &account);
    ...
    #[test(dr = @DiemRoot, tc = @TreasuryCompliance, account = @0x100)]
    #[expected_failure(abort_code = 7, location = SharedEd25519PublicKey)]
    fun rotate_to_key_with_good_length_but_bad_contents(
        dr: signer, tc: signer, account: signer
    ) {
        setup(&dr, &tc, &account);
    ...
    #[test(dr = @DiemRoot, tc = @TreasuryCompliance, account = @0x100)]
    #[expected_failure(abort_code = 261, location = SharedEd25519PublicKey)]
    fun rotate_key_on_non_shared_account(
        dr: signer, tc: signer, account: signer
    ) {
        setup(&dr, &tc, &account);
