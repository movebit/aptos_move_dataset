...
module std::vault {
    ...
    public fun read_cap_type(): CapType {
        CapType { code: 0 }
    }

    /// Creates a modify  capability type.
    public fun modify_cap_type(): CapType {
        CapType { code: 1 }
    }

    /// Creates a delegate  capability type.
    public fun delegate_cap_type(): CapType {
        CapType { code: 2 }
    }

    /// Creates a transfer  capability type.
    public fun transfer_cap_type(): CapType {
        CapType { code: 3 }
    }
    ...
    /// the vault in events.
    public fun enable_events<Content: store>(
    ...
    fun validate_cap<Content: store + drop>(
        requester: &signer, cap: CapType
    ): (address, address) acquires VaultDelegate {
    ...
    public fun read_accessor<Content: store + drop>(
        ...
        ReadAccessor {
            ...
            content: option::extract(content)
        }
    }

    /// Returns a reference to the content represented by a read accessor.
    public fun borrow<Content: store + drop>(
        accessor: &ReadAccessor<Content>
    ): &Content {
        &accessor.content
    }

    /// Releases read accessor.
    public fun release_read_accessor<Content: store + drop>(
        accessor: ReadAccessor<Content>
    ) acquires Vault {
    ...
    public fun borrow_mut<Content: store + drop>(
        accessor: &mut ModifyAccessor<Content>
    ): &mut Content {
    ...
    /// to the vault.
    public fun release_modify_accessor<Content: store + drop>(
        accessor: ModifyAccessor<Content>
    ) acquires Vault {
    ...
    public fun delegate<Content: store + drop>(
        cap: &DelegateCap<Content>, to_signer: &signer, cap_type: CapType
    ...
    public fun revoke<Content: store + drop>(
        cap: &DelegateCap<Content>, addr: address, cap_type: CapType
    ) acquires VaultDelegates, VaultDelegate, VaultEvents {
        assert!(
            exists<VaultDelegates<Content>>(cap.vault_address),
            error::invalid_state(EDELEGATION_NOT_ENABLED)
    ...
    public fun transfer<Content: store + drop>(
        ...
        if (exists<VaultDelegates<Content>>(cap.vault_address)) {
            let delegate_cap = DelegateCap<Content> {
                vault_address: cap.vault_address,
                authority: cap.authority
            };
            revoke_all(&delegate_cap);
