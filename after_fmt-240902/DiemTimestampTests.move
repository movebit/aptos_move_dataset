...
module DiemFramework::DiemTimestampTests {
    ...
    fun update_global_time_post_genesis_vm_nil_proposer_equal_timestamp(
        dr: signer, tc: signer, vm: signer
    ) {
        Genesis::setup(&dr, &tc);
    ...
    fun update_global_time_post_genesis_vm_nil_proposer_increasing_timestamp(
        dr: signer, tc: signer, vm: signer
    ) {
        Genesis::setup(&dr, &tc);
        assert!(DiemTimestamp::now_microseconds() == 0, 0);
    ...
    #[test(dr = @DiemRoot, tc = @TreasuryCompliance, vm = @VMReserved)]
    #[expected_failure(abort_code = 519, location = DiemTimestamp)]
    fun update_global_time_post_genesis_vm_not_nil_proposer_equal_timestamp(
        dr: signer, tc: signer, vm: signer
    ) {
        Genesis::setup(&dr, &tc);
        assert!(DiemTimestamp::now_microseconds() == 0, 0);
    ...
    #[test(dr = @DiemRoot, tc = @TreasuryCompliance, vm = @VMReserved)]
    fun update_global_time_post_genesis_vm_not_nil_proposer_increasing_timestamp(
        dr: signer, tc: signer, vm: signer
    ) {
        Genesis::setup(&dr, &tc);
        assert!(DiemTimestamp::now_microseconds() == 0, 0);
