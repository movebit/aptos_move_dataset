...
module DiemFramework::AccountAdministrationScripts {
    ...
    public entry fun add_currency_to_account<Currency>(account: signer) {
    ...
    spec add_currency_to_account {
        ...
        include DiemAccount::AddCurrencyEnsures<Currency> {
            addr: signer::address_of(account)
        };

        aborts_with [check] errors::NOT_PUBLISHED, errors::INVALID_ARGUMENT, errors::ALREADY_PUBLISHED;
    ...
    public entry fun add_recovery_rotation_capability(
        to_recover_account: signer, recovery_address: address
    ) {
    ...
    public entry fun publish_shared_ed25519_public_key(
        account: signer, public_key: vector<u8>
    ) {
    ...
    spec publish_shared_ed25519_public_key {
        ...
        aborts_with [check] errors::INVALID_STATE, errors::ALREADY_PUBLISHED, errors::INVALID_ARGUMENT;
    }
    ...
    /// corresponds to an ed25519 public key,
    ...
    spec rotate_authentication_key {
        ...
        include DiemAccount::RotateAuthenticationKeyAbortsIf {
            cap: key_rotation_capability,
            new_authentication_key: new_key
        };

        /// This rotates the authentication key of `account` to `new_key`
        include DiemAccount::RotateAuthenticationKeyEnsures {
            addr: account_addr,
            new_authentication_key: new_key
        };

        aborts_with [check] errors::INVALID_STATE, errors::INVALID_ARGUMENT;
    ...
    spec rotate_authentication_key_with_nonce {
        ...
        aborts_with [check] errors::INVALID_ARGUMENT, errors::INVALID_STATE, errors::NOT_PUBLISHED;
    ...
    spec rotate_authentication_key_with_nonce_admin {
        ...
        include DiemAccount::TransactionChecks { sender: account }; // properties checked by the prologue.
        let account_addr = signer::address_of(account);
        include SlidingNonce::RecordNonceAbortsIf {
            account: dr_account,
            seq_nonce: sliding_nonce
        };
        include DiemAccount::ExtractKeyRotationCapabilityAbortsIf;
        let key_rotation_capability = DiemAccount::spec_get_key_rotation_cap(account_addr);
        include DiemAccount::RotateAuthenticationKeyAbortsIf {
    ...
    /// published under `recovery_address`. `new_key` must be a valide authentication key .
    ...
    public entry fun rotate_authentication_key_with_recovery_address(
        account: signer,
    ...
    spec rotate_authentication_key_with_recovery_address {
        ...
        aborts_with [check] errors::NOT_PUBLISHED, errors::INVALID_ARGUMENT;
        ...
        let account_addr = signer::address_of(account);
        aborts_if !RecoveryAddress::spec_holds_key_rotation_cap_for(
            recovery_address, to_recover
    ...
    public entry fun rotate_dual_attestation_info(
        account: signer, new_url: vector<u8>, new_key: vector<u8>
    ) {
    ...
    /// rotates the `account`'s authentication key to the new authentication key derived from `public_key`
    ...
    public entry fun rotate_shared_ed25519_public_key(
        account: signer, public_key: vector<u8>
    ) {
    ...
    public entry fun create_recovery_address(account: signer) {
        RecoveryAddress::publish(
            &account, DiemAccount::extract_key_rotation_capability(&account)
        )
    }
    ...
    public entry fun create_vasp_domains(account: signer) {
    ...
    spec create_vasp_domains {
        ...
        aborts_with [check] errors::ALREADY_PUBLISHED, errors::REQUIRES_ROLE;
    }
    ...
    public entry fun opt_in_to_crsn(account: signer, crsn_size: u64) {
    ...
    public entry fun force_expire(account: signer, shift_amount: u64) {
