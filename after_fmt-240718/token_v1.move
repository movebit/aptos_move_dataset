module 0xABCD::token_v1 {
    ...
    public entry fun token_v1_initialize_collection(creator: &signer) {
        ...
        move_to(
            creator,
            MinterConfig { signer_cap, minted_tokens: table::new(), tokendata_id },
        );
    }

    fun get_signer(account_address: address): signer acquires MinterConfig {
        account::create_signer_with_capability(
            &borrow_global<MinterConfig>(account_address).signer_cap
        )
    }

    fun set_token_minted(
        user_address: address, creator_address: address, token: Token
    ) acquires MinterConfig {
        let minted_tokens =
            &mut borrow_global_mut<MinterConfig>(creator_address).minted_tokens;
    ...
    fun mint_nft_sequential(creator_address: address): (signer, TokenId) acquires MinterConfig {
        ...
        let current_supply_opt =
            token::get_collection_supply(
                resource_signer_address, string::utf8(COLLECTION_NAME)
            );
    ...
    fun mint_nft_parallel(user: &signer, creator_address: address): (signer, TokenId) acquires MinterConfig {
        ...
        let token_name =
            build_token_name(
                ...
                account::get_sequence_number(signer::address_of(user)),
            );
        let tokendata_id =
            token_v1_create_token_data(
                &resource_signer,
    ...
    public entry fun token_v1_mint_and_store_nft_sequential(
    ...
    public entry fun token_v1_mint_and_transfer_nft_sequential(
        user: &signer, creator_address: address
    ) acquires MinterConfig {
        let (resource_signer, token_id) = mint_nft_sequential(creator_address);
    ...
    /// Mint NFT and store it in a table
    public entry fun token_v1_mint_and_store_nft_parallel(
        user: &signer, creator_address: address
    ) acquires MinterConfig {
    ...
    /// Mint NFT and transfer it to the user
    public entry fun token_v1_mint_and_transfer_nft_parallel(
        user: &signer, creator_address: address
    ) acquires MinterConfig {
        let (resource_signer, token_id) = mint_nft_parallel(user, creator_address);
        token::direct_transfer(&resource_signer, user, token_id, 1);
    ...
    public entry fun token_v1_mint_and_store_ft(
        user: &signer, creator_address: address
    ) acquires MinterConfig {
        let resource_signer = &get_signer(creator_address);
    ...
    public entry fun token_v1_mint_and_transfer_ft(
        user: &signer, creator_address: address
    ) acquires MinterConfig {
        let resource_signer = &get_signer(creator_address);
        let tokendata_id = borrow_global<MinterConfig>(creator_address).tokendata_id;
