...
module 0x1::NFTTests {
    ...
    public entry fun test_all(admin: signer, creator: signer, user: signer) {
        ...
        */
        ...
        assert!(
            NFT::get_metadata(&token1) == Game { name: b"Mario", edition: 2008 },
            EMINT_FAILED
        );
        ...
        assert!(
            NFTGallery::get_token_balance<Game>(creator_addr, &token1_id) == 10,
            EMINT_FAILED
        );
        assert!(
            NFTGallery::get_token_balance<Game>(creator_addr, &token1_id) == 10,
        ...
        assert!(
            NFTGallery::get_token_balance<Game>(creator_addr, &token2_id) == 233,
            EMINT_FAILED
        );
        ...
        */
        ...
        assert!(
            NFTGallery::get_token_balance<Pokemon>(creator_addr, &pikachu_id) == 10,
            ECOLLECTION_FAILED
        );
        assert!(
            NFTGallery::get_token_supply<Pokemon>(creator_addr, &pikachu_id) == 10,
            ECOLLECTION_FAILED
        );
        assert!(
            NFTGallery::get_token_content_uri<Pokemon>(creator_addr, &pikachu_id)
                == b"nintendo.com",
            ECOLLECTION_FAILED
        );
        assert!(
            NFTGallery::get_token_metadata<Pokemon>(creator_addr, &pikachu_id)
                == Pokemon { name: b"Pikachu", type: b"electric" },
            ECOLLECTION_FAILED
        );
        assert!(
            NFTGallery::get_token_parent_id<Pokemon>(creator_addr, &pikachu_id)
                == option::some(NFT::id(&collection)),
            ECOLLECTION_FAILED
        );
        ...
        */
        ...
        assert!(
            NFTGallery::get_token_balance<Game>(creator_addr, &token1_id) == 4,
            ETRANSFER_FAILED
        );
        assert!(NFTGallery::has_token<Game>(user_addr, &token1_id), ETRANSFER_FAILED);
        assert!(
            NFTGallery::get_token_balance<Game>(user_addr, &token1_id) == 6,
            ETRANSFER_FAILED
        );
        assert!(
            NFTGallery::get_token_supply<Game>(user_addr, &token1_id) == 10,
            ETRANSFER_FAILED
        ...
        */
        ...
        assert!(
            NFTGallery::get_token_balance<Game>(user_addr, &token1_id) == 0,
            ETRANSFER_FAILED
        );
        assert!(NFTGallery::has_token<Game>(creator_addr, &token1_id), ETRANSFER_FAILED);
        assert!(
        ...
        */
        ...
        assert!(
            NFT::get_metadata(&nft) == Game { name: b"Mario", edition: 2008 },
            EMINT_FAILED
        );
