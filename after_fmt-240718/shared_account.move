...
module shared_account::SharedAccount {
    ...
    public entry fun disperse<CoinType>(resource_addr: address) acquires SharedAccount {
        ...
        let resource_signer =
            account::create_signer_with_capability(&shared_account.signer_capability);

        vector::for_each_ref(
            &shared_account.share_record,
            |shared_record| {
    ...
    #[test_only]
    public fun set_up(
        user: signer, test_user1: signer, test_user2: signer
    ): address acquires SharedAccountEvent {
