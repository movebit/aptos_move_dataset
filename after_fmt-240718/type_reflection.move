...
module 0x42::test {
    ...
    fun test_type_info_symbolic<T>(): type_info::TypeInfo {
        spec {
            assert type_info::type_of<T>().account_address
                == type_info::type_of<T>().account_address;
            assert type_info::type_of<T>().module_name
                == type_info::type_of<T>().module_name;
            assert type_info::type_of<T>().struct_name
                == type_info::type_of<T>().struct_name;
        };
...
module 0x43::test {
    ...
    spec test_type_name_concrete_struct {
        ensures result.bytes
            == b"00000000000000000000000000000043::test::Pair<address, bool>";
    }
}
