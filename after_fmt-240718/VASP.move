...
module DiemFramework::VASP {
    ...
    public(friend) fun publish_parent_vasp_credential(
        vasp: &signer, tc_account: &signer
    ) {
    ...
    spec publish_child_vasp_credential {
        ...
        include PublishChildVASPEnsures {
    ...
    public fun is_same_vasp(addr1: address, addr2: address): bool acquires ChildVASP {
        is_vasp(addr1)
            && is_vasp(addr2)
            && parent_address(addr1) == parent_address(addr2)
    }
    ...
    spec fun spec_is_same_vasp(addr1: address, addr2: address): bool {
        is_vasp(addr1)
            && is_vasp(addr2)
            && spec_parent_address(addr1) == spec_parent_address(addr2)
    }
    ...
    spec module {
        invariant update forall addr: address where old(is_parent(addr)): is_parent(addr);

        invariant update forall addr: address where old(is_child(addr)): is_child(addr);
    }
