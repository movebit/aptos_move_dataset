...
module defi::locked_coins {
    ...
    public fun total_locks<CoinType>(sponsor: address): u64 acquires Locks {
        assert!(
            exists<Locks<CoinType>>(sponsor),
            error::not_found(ESPONSOR_ACCOUNT_NOT_INITIALIZED),
        );
    ...
    public fun locked_amount<CoinType>(
        sponsor: address, recipient: address
    ): u64 acquires Locks {
        assert!(
            exists<Locks<CoinType>>(sponsor),
            error::not_found(ESPONSOR_ACCOUNT_NOT_INITIALIZED),
        ...
        let locks = borrow_global<Locks<CoinType>>(sponsor);
        assert!(
            table::contains(&locks.locks, recipient), error::not_found(ELOCK_NOT_FOUND)
        );
    ...
    /// This throws an error if there are no locked coins setup for the given recipient.
    public fun claim_time_secs<CoinType>(
        sponsor: address, recipient: address
    ): u64 acquires Locks {
        assert!(
            exists<Locks<CoinType>>(sponsor),
    ...
    public entry fun initialize_sponsor<CoinType>(
        sponsor: &signer, withdrawal_address: address
    ) {
        move_to(
            sponsor,
            Locks {
    ...
    public entry fun update_withdrawal_address<CoinType>(
        ...
        assert!(
            exists<Locks<CoinType>>(sponsor_address),
            error::not_found(ESPONSOR_ACCOUNT_NOT_INITIALIZED),
        );
    ...
    public entry fun batch_add_locked_coins<CoinType>(
        ...
        assert!(
            ...
            error::invalid_argument(EINVALID_RECIPIENTS_LIST_LENGTH),
        );
    ...
    public entry fun add_locked_coins<CoinType>(
        ...
        assert!(
            !table::contains(&locks.locks, recipient),
            error::already_exists(ELOCK_ALREADY_EXISTS),
        );
        table::add(
            &mut locks.locks, recipient, Lock<CoinType> { coins, unlock_time_secs }
        );
    ...
    public entry fun claim<CoinType>(recipient: &signer, sponsor: address) acquires Locks {
        ...
        assert!(
            table::contains(&locks.locks, recipient_address),
            error::not_found(ELOCK_NOT_FOUND),
        );
        ...
        let Lock { coins, unlock_time_secs } = table::remove(
            &mut locks.locks, recipient_address
        );
        locks.total_locks = locks.total_locks - 1;
        let now_secs = timestamp::now_seconds();
        assert!(
            now_secs >= unlock_time_secs, error::invalid_state(ELOCKUP_HAS_NOT_EXPIRED)
        );
    ...
    public entry fun batch_cancel_lockup<CoinType>(
        sponsor: &signer, recipients: vector<address>
    ) acquires Locks {
        let sponsor_address = signer::address_of(sponsor);
        assert!(
            exists<Locks<CoinType>>(sponsor_address),
    ...
    public entry fun cancel_lockup<CoinType>(
        sponsor: &signer, recipient: address
    ) acquires Locks {
        let sponsor_address = signer::address_of(sponsor);
        assert!(
            exists<Locks<CoinType>>(sponsor_address),
        ...
        let Lock { coins, unlock_time_secs: _ } = table::remove(
            &mut locks.locks, recipient
        );
        locks.total_locks = locks.total_locks - 1;
        let amount = coin::value(&coins);
        ...
        event::emit(
            CancelLockup { sponsor: sponsor_address, recipient, amount },
        );
    }
    ...
    public entry fun test_sponsor_can_batch_cancel_lockup(
        ...
        batch_cancel_lockup<AptosCoin>(
            sponsor, vector[recipient_1_addr, recipient_2_addr]
        );
        let locks = borrow_global_mut<Locks<AptosCoin>>(sponsor_address);
