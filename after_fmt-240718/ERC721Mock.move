...
module Evm::ERC721 {
    ...
    #[callable(sig = b"safeTransferFrom(address,address,uint256,bytes)")]
    ...
    fun setApprovalForAll_(
        owner: address, operator: address, approved: bool
    ) acquires State {
    ...
    fun mut_operatorApproval(
        s: &mut State, owner: address, operator: address
    ): &mut bool {
    ...
    fun doSafeTransferAcceptanceCheck(
        ...
        if (isContract(to)) {
            let result =
                IERC721Receiver_try_call_onERC721Received(
                    to, sender(), from, tokenId, data
                );
