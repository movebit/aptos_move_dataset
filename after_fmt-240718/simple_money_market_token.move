...
module Token {
    ...
    public fun withdraw<ATy: copy + drop + store>(
        coin: &mut Coin<ATy>, amount: u64
    ): Coin<ATy> {
    ...
    public fun deposit<ATy: copy + drop + store>(
...
module OneToOneMarket {
    ...
    public fun deposit<In: copy + drop + store, Out: copy + drop + store>(
        account: &signer, coin: Token::Coin<In>
    ...
    fun max_borrow_amount<In: copy + drop + store, Out: copy + drop + store>(
        ...
        let input_into_output = input_deposited
            * borrow_global<Price<In, Out>>(@0xB055).price;
        let max_output =
