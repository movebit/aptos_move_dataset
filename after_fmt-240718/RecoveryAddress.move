...
module DiemFramework::RecoveryAddress {
    ...
    public fun publish(
        recovery_account: &signer, rotation_cap: KeyRotationCapability
    ) {
    ...
    spec schema PublishAbortsIf {
        ...
        aborts_if DiemAccount::key_rotation_capability_address(rotation_cap) != addr with errors::INVALID_ARGUMENT;
    }
    ...
    public fun add_rotation_capability(
        to_recover: KeyRotationCapability, recovery_address: address
    ) acquires RecoveryAddress {
        // Check that `recovery_address` has a `RecoveryAddress` resource
        assert!(
            exists<RecoveryAddress>(recovery_address),
    ...
    spec schema AddRotationCapabilityAbortsIf {
        ...
        recovery_address: address;
        aborts_if !spec_is_recovery_address(recovery_address) with errors::NOT_PUBLISHED;
        aborts_if len(global<RecoveryAddress>(recovery_address).rotation_caps)
            >= MAX_REGISTERED_KEYS with errors::LIMIT_EXCEEDED;
