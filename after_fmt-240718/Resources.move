module 0x42::TestResources {
    ...
    spec move_from_addr_to_sender {
        aborts_if !exists<R>(a);
        aborts_if (signer::address_of(account) != a)
            && exists<R>(signer::address_of(account));
        ensures exists<R>(signer::address_of(account));
    ...
    spec move_from_sender_to_sender {
        aborts_if !exists<R>(signer::address_of(account));
        ensures exists<R>(signer::address_of(account));
        ensures old(global<R>(signer::address_of(account)).x)
            == global<R>(signer::address_of(account)).x;
        ensures old(global<R>(signer::address_of(account)))
            == global<R>(signer::address_of(account));
    }
