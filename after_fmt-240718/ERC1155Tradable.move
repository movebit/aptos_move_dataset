...
module Evm::ERC1155Tradable {
    ...
    public fun burnBatch(
        owner: address, ids: vector<U256>, amounts: vector<U256>
    ) acquires State {
    ...
    public fun constructor(
        name: vector<u8>, symbol: vector<u8>, baseURI: vector<u8>
    ) acquires State {
    ...
    public fun balanceOfBatch(
        accounts: vector<address>, ids: vector<U256>
    ...
    fun mut_operatorApprovals(
        s: &mut State, account: address, operator: address
    ): &mut bool {
    ...
    fun doSafeBatchTransferAcceptanceCheck(
        operator: address,
        from: address,
        to: address,
        ids: vector<U256>,
