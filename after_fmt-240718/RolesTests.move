...
module DiemFramework::RolesTests {
    ...
    #[test(tc = @TreasuryCompliance, dr = @DiemRoot)]
    #[expected_failure(abort_code = 1, location = DiemFramework::DiemTimestamp)]
    fun grant_treasury_compliance_wrong_addr_post_genesis(
        tc: signer, dr: signer
    ) {
        Genesis::setup(&dr, &tc);
        let account = get_account();
    ...
    #[test(tc = @TreasuryCompliance, dr = @DiemRoot)]
    #[expected_failure(abort_code = 1, location = DiemFramework::DiemTimestamp)]
    fun grant_treasury_compliance_wrong_granting_addr_post_genesis(
        tc: signer, dr: signer
    ) {
        Genesis::setup(&dr, &tc);
    ...
    #[test(tc = @TreasuryCompliance, dr = @DiemRoot)]
    #[expected_failure(abort_code = 2, location = DiemFramework::CoreAddresses)]
    fun grant_validator_operator_role_non_dr_granter(
        tc: signer, dr: signer
    ) {
        Genesis::setup(&dr, &tc);
        let account = get_account();
    ...
    #[test(tc = @TreasuryCompliance, dr = @DiemRoot)]
    #[expected_failure(abort_code = 6, location = Roles)]
    fun double_grant_validator_operator_role_dr_granter(
        tc: signer, dr: signer
    ) {
        Genesis::setup(&dr, &tc);
        let account = get_account();
    ...
    fun grant_child_vasp_role_non_parent_vasp_granter(
        tc: signer, dr: signer
    ) {
        Genesis::setup(&dr, &tc);
        let account = get_account();
