module ExperimentalFramework::MultiTokenBalance {
    ...
    spec TokenBalance {
        invariant forall i1 in range(gallery), i2 in range(gallery) where gallery[i1].id
            == gallery[i2].id: i1 == i2;
    }
    ...
    spec add_to_gallery {
        ...
        ensures is_in_gallery(gallery, token.id) ==>
            len(gallery) == len(post_gallery);
        ensures !is_in_gallery(gallery, token.id) ==>
            len(gallery) + 1 == len(post_gallery);

        ensures is_in_gallery(gallery, token.id) ==>
            post_gallery[len(gallery) - 1].balance
    ...
    fun index_of_token<TokenType: store>(
    ...
    public fun has_token<TokenType: store>(
    ...
    public fun get_token_balance<TokenType: store>(
        owner: address, token_id: &guid::ID
    ...
    spec get_token_balance {
        ...
        ensures is_in_gallery(gallery, token_id) ==>
            result == gallery[min_token_idx].balance;
    }
    ...
    spec transfer_multi_token_between_galleries {
        ...
        aborts_if owner != to
            && is_in_gallery(gallery_to, id)
            && MAX_U64 - amount < gallery_to[min_token_idx_to].balance;
