...
module Escape {
    ...
    struct Wrapper<Thing: key + store> has key {
        thing: Thing
    }
    ...
    invariant forall addr: address where exists<Wrapper<IndoorThing>>(addr): addr == @
        0x123;

    invariant forall addr: address where exists<Wrapper<OutdoorThing>>(addr): addr == @
        0x123;
}
}
