...
module DiemFramework::DesignatedDealer {
    ...
    public(friend) fun add_currency<CoinType>(
        dd: &signer, tc_account: &signer
    ) {
        Roles::assert_treasury_compliance(tc_account);
        let dd_addr = signer::address_of(dd);
    ...
    public fun add_currency_for_test<CoinType>(
        dd: &signer, tc_account: &signer
    ) {
        add_currency<CoinType>(dd, tc_account)
    }
    ...
    spec tiered_mint {
        ...
        ensures post_currency_info
            == update_field(
                currency_info, total_value, currency_info.total_value + amount
            );
    }
