...
module DiemFramework::NetworkIdentity {
    ...
    public fun initialize_network_identity_event_handle(
        tc_account: &signer
    ) {
    ...
    public fun add_identities(
        account: &signer, to_add: vector<vector<u8>>
    ...
    public fun remove_identities(
        account: &signer, to_remove: vector<vector<u8>>
    ) acquires NetworkIdentity, NetworkIdentityEventHandle {
        assert!(
            tc_network_identity_event_handle_exists(),
            errors::not_published(ENETWORK_ID_EVENT_HANDLE_INVALID),
