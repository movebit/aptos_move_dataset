...
module NFT {
    ...
    public fun get_balance<TokenType: copy + store + drop>(
    ...
    public fun get_supply<TokenType: copy + store + drop>(
        token: &Token<TokenType>
    ): u64 acquires TokenDataCollection {
        if (option::is_some(&token.token_data)) {
            option::borrow(&token.token_data).supply
        } else {
            let creator_addr = guid::id_creator_address(&token.id);
            let creator_tokens_data =
                &borrow_global<TokenDataCollection<TokenType>>(creator_addr).tokens;
            let token_data_idx =
                *option::borrow(
                    &index_of_token<TokenType>(creator_tokens_data, &token.id)
                );
    ...
    public fun get_content_uri<TokenType: copy + store + drop>(
        token: &Token<TokenType>
    ): vector<u8> acquires TokenDataCollection {
        if (option::is_some(&token.token_data)) {
    ...
    public fun get_metadata<TokenType: copy + store + drop>(
        token: &Token<TokenType>
    ): TokenType acquires TokenDataCollection {
        if (option::is_some(&token.token_data)) {
    ...
    /// Returns a copy of the token metadata
    public fun get_parent_id<TokenType: copy + store + drop>(
        token: &Token<TokenType>
    ): Option<guid::ID> acquires TokenDataCollection {
        if (option::is_some(&token.token_data)) {
    ...
    public fun is_data_inlined<TokenType: copy + store + drop>(
        token: &Token<TokenType>
    ): bool {
    ...
    fun index_of_token<TokenType: copy + store + drop>(
        gallery: &vector<TokenData<TokenType>>, id: &guid::ID
    ): Option<u64> {
        let i = 0;
    ...
    public fun join<TokenType: copy + store + drop>(
        token: &mut Token<TokenType>, other: Token<TokenType>
    ) {
    ...
    public fun split_out<TokenType: copy + store + drop>(
        token: &mut Token<TokenType>, amount: u64
    ): Token<TokenType> {
    ...
    fun create_impl<TokenType: copy + store + drop>(
        ...
        if (amount == 1) {
            ...
            let token_data_collection =
                &mut borrow_global_mut<TokenDataCollection<TokenType>>(addr).tokens;
            vector::push_back(token_data_collection, token_data);
            Token { id, balance: amount, token_data: option::none() }
        }
    }

    public fun publish_token_data_collection<TokenType: copy + store + drop>(
        account: &signer
    ) {
        assert!(
    ...
    public fun allow_creation_delegation<TokenType: copy + store + drop>(
        account: &signer
    ) {
