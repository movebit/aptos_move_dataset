...
module ExperimentalFramework::Vote {
    ...
    public fun create_ballot<Proposal: store + copy + drop>(
        ...
        assert!(
            DiemTimestamp::now_seconds() < expiration_timestamp_secs,
            errors::invalid_argument(EINVALID_TIMESTAMP)
        );
        ...
        if (!exists<Ballots<Proposal>>(ballot_address)) {
            move_to(
                ...
                Ballots<Proposal> {
                    ...
                    create_ballot_handle: event::new_event_handle<CreateBallotEvent<Proposal>>(
                        ballot_account
                    ),
                    remove_ballot_handle: event::new_event_handle<RemoveBallotEvent>(
                        ballot_account
                    ),
                    voted_handle: event::new_event_handle<VotedEvent>(ballot_account),
                    ballot_approved_handle: event::new_event_handle<BallotApprovedEvent>(
                        ballot_account
                    )
                }
            );
        ...
        assert!(
            vector::length(ballots) < MAX_BALLOTS_PER_PROPOSAL_TYPE_PER_ADDRESS,
            errors::limit_exceeded(ETOO_MANY_BALLOTS)
        );
        ...
        event::emit_event<CreateBallotEvent<Proposal>>(
            ...
            CreateBallotEvent { ballot_id: *&ballot_id, ballot }
        );
        ballot_id
    }
    ...
    fun check_voter_present(
        weighted_voters: &vector<WeightedVoter>, voter: &vector<u8>
    ): bool {
        let i = 0;
    ...
    public fun vote<Proposal: store + drop>(
        ...
        assert!(
            ...
            errors::invalid_argument(EBALLOT_PROPOSAL_MISMATCH)
        );
        assert!(
            &ballot.proposal_type == &proposal_type,
            errors::invalid_argument(EBALLOT_PROPOSAL_MISMATCH)
        );
        ...
        assert!(
            check_voter_present(allowed_voters, &voter_address_bcs),
            errors::invalid_state(EINVALID_VOTER)
        );
        assert!(
            DiemTimestamp::now_seconds() <= ballot.expiration_timestamp_secs,
            errors::invalid_state(EBALLOT_EXPIRED)
        );

        assert!(
            !check_voter_present(&ballot.votes_received, &voter_address_bcs),
            errors::invalid_state(EALREADY_VOTED)
        );
        ...
        while (i < len) {
            ...
            if (&weighted_voter.voter == &voter_address_bcs) {
                ...
                ballot.total_weighted_votes_received = ballot.total_weighted_votes_received
                    + weighted_voter.weight;
        ...
        let ballot_approved =
            ballot.total_weighted_votes_received >= ballot.num_votes_required;
    ...
    public(friend) fun remove_ballot_internal<Proposal: store + drop>(
        account: signer, ballot_id: BallotID
    ) acquires Ballots {
    ...
    spec module {
        ...
        invariant update forall ballot_addr: address where old(
            exists<BallotCounter>(ballot_addr)
        ): exists<BallotCounter>(ballot_addr);

        /// Once a proposal is initialized, it stays initialized forever.
        invariant<Proposal> update forall ballot_addr: address where old(
    ...
    spec fun unique_ballots<Proposal>(ballots: vector<Ballot<Proposal>>): bool {
        forall i in 0..len(ballots),
        j in 0..len(ballots): ballots[i].ballot_id == ballots[j].ballot_id ==> i == j
    }
    ...
    spec fun ballot_id_does_not_exist<Proposal>(
        ballot_id: BallotID, ballots: vector<Ballot<Proposal>>, i: u64
    ): bool {
    ...
    spec remove_ballot_internal {
        ...
        ensures ballot_id_does_not_exist<Proposal>(ballot_id, ballots, len(ballots));
    }
