module 0x1::aggregator_v2_test {
    ...
    public entry fun init_aggregator<Element: store + drop>(
        account: &signer, use_type: u32
    ) {
        init<Aggregator<Element>>(account, use_type);
    }

    public entry fun init_snapshot<Element: store + drop>(
        account: &signer, use_type: u32
    ) {
        init<AggregatorSnapshot<Element>>(account, use_type);
    }

    public entry fun init_derived_string<Element: store>(
        account: &signer, use_type: u32
    ) {
    ...
    fun delete<Agg: store + drop>(account_addr: address, use_type: u32) acquires AggregatorInResource, AggregatorInResourceGroup {
        if (use_type == USE_RESOURCE_TYPE) {
            assert!(
                exists<AggregatorInResource<Agg>>(account_addr), ERESOURCE_DOESNT_EXIST
            );
    ...
    public entry fun delete_aggregator<Element: store + drop>(
    ...
    public entry fun delete_snapshot<Element: store + drop>(
        account_addr: address, use_type: u32
    ) acquires AggregatorInResource, AggregatorInResourceGroup {
        delete<AggregatorSnapshot<Element>>(account_addr, use_type);
    }

    public entry fun delete_derived_string<Element: store + drop>(
        account_addr: address, use_type: u32
    ) acquires AggregatorInResource, AggregatorInResourceGroup {
    ...
    public entry fun new<Element: drop + copy + store>(
        ...
        insert<Aggregator<Element>>(
            addr, use_type, i, aggregator_v2::create_aggregator(limit)
        );
    }

    public fun call_read<Element: store + drop>(
        addr: address, use_type: u32, i: u64
    ): Element acquires AggregatorInResource, AggregatorInTable, AggregatorInResourceGroup {
        for_element_ref<Aggregator<Element>, Element>(
            addr, use_type, i, |aggregator| aggregator_v2::read(aggregator)
        )
    }
    ...
    public entry fun materialize<Element: store + drop>(
        addr: address, use_type: u32, i: u64
    ) acquires AggregatorInResource, AggregatorInTable, AggregatorInResourceGroup {
    ...
    public entry fun add_2<A: store + drop, B: store + drop>(
        addr_a: address,
        use_type_a: u32,
        i_a: u64,
        a: A,
    ...
    public entry fun snapshot<Element: store + drop>(
        addr_i: address,
        use_type_i: u32,
        i: u64,
        addr_j: address,
        ...
        let snapshot =
            for_element_ref<Aggregator<Element>, AggregatorSnapshot<Element>>(
                addr_i,
                use_type_i,
                i,
    ...
    public entry fun concat<Element: store + drop>(
        addr_i: address,
        use_type_i: u32,
        i: u64,
        addr_j: address,
        ...
        let snapshot =
            for_element_ref<AggregatorSnapshot<Element>, DerivedStringSnapshot>(
                addr_i,
                use_type_i,
                i,
                |snapshot| {
    ...
    public entry fun read_snapshot<Element: store + drop>(
        addr: address, use_type: u32, i: u64
    ) acquires AggregatorInResource, AggregatorInTable, AggregatorInResourceGroup {
        for_element_ref<AggregatorSnapshot<Element>, Element>(
            addr, use_type, i, |snapshot| aggregator_v2::read_snapshot(snapshot)
        );
    }
    ...
    public entry fun check_derived<Element: store + drop>(
        ...
        let actual =
            for_element_ref<DerivedStringSnapshot, String>(
                addr, use_type, i, |snapshot| aggregator_v2::read_derived_string(snapshot)
            );
        assert!(actual == expected, ENOT_EQUAL)
    }
    ...
    public entry fun add_and_read_snapshot<Element: copy + store + drop>(
        ...
        for_element_mut<Aggregator<Element>, Element>(
            ...
            |aggregator| {
                ...
                let _snapshot_value_1 = aggregator_v2::read_snapshot(
                    &aggregator_snapshot_1
                );
                let _snapshot_value_2 = aggregator_v2::read_snapshot(
                    &aggregator_snapshot_2
                );
                let snapshot_value_3 = aggregator_v2::read_snapshot(
                    &aggregator_snapshot_3
                );
