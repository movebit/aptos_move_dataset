...
module knight::food {
    ...
    public fun food_token_address(food_token_name: String): address {
        token::create_token_address(
            &@knight, &string::utf8(FOOD_COLLECTION_NAME), &food_token_name
        )
    }

    /// Mints the given amount of the corn token to the given receiver.
    public entry fun mint_corn(
        creator: &signer, receiver: address, amount: u64
    ) acquires FoodToken {
    ...
    public(friend) fun burn_food(
        from: &signer, food: Object<FoodToken>, amount: u64
    ) acquires FoodToken {
        let metadata = object::convert<FoodToken, Metadata>(food);
