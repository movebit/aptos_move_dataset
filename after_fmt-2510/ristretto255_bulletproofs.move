...
module aptos_std::ristretto255_bulletproofs {
    ...
    public fun verify_batch_range_proof_pedersen(
        ...
        verify_batch_range_proof(
            &comms.map_ref(
                |com| ristretto255::point_clone(pedersen::commitment_as_point(com))
            ),
            &ristretto255::basepoint(),
            &ristretto255::hash_to_point_base(),
            proof,
    ...
    public fun verify_batch_range_proof(
        ...
        let comms =
            comms.map_ref(
                |com| ristretto255::point_to_bytes(&ristretto255::point_compress(com))
            );
    ...
    fun test_batch_prover(fx: signer) {
        ...
        assert!(
            verify_batch_range_proof_pedersen(&comms, &proof, 64, A_DST) == false,
            1
        );
        assert!(
            verify_batch_range_proof_pedersen(&comms, &proof, 32, A_DST) == false,
            1
        );
        assert!(
            verify_batch_range_proof_pedersen(&comms, &proof, 16, A_DST) == false,
            1
        );
        assert!(
