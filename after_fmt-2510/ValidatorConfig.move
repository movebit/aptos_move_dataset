...
module DiemFramework::ValidatorConfig {
    ...
    public(friend) fun publish(
        validator_account: &signer, dr_account: &signer, human_name: vector<u8>
    ) {
    ...
    spec schema PublishAbortsIf {
        ...
        aborts_if exists_config(validator_addr) with errors::ALREADY_PUBLISHED;
    }
    ...
    public fun set_operator(
        validator_account: &signer, operator_addr: address
        ...
        (borrow_global_mut<ValidatorConfig>(sender)).operator_account = option::some(
            operator_addr
        );
    }
    ...
    spec schema SetOperatorAbortsIf {
        ...
        let validator_addr = signer::address_of(validator_account);
        include Roles::AbortsIfNotValidator { account: validator_account };
        aborts_if !ValidatorOperatorConfig::has_validator_operator_config(operator_addr) with errors::INVALID_ARGUMENT;
    ...
    spec schema SetOperatorEnsures {
        ...
        ensures forall addr: address where addr != validator_addr:
            global<ValidatorConfig>(addr).operator_account
                == old(global<ValidatorConfig>(addr).operator_account);
    }
    ...
    spec set_config {
        ...
        ensures global<ValidatorConfig>(validator_addr)
            == update_field(
                old(global<ValidatorConfig>(validator_addr)),
                config,
                option::spec_some(
                    Config {
                        consensus_pubkey,
    ...
    spec schema SetConfigAbortsIf {
        ...
        aborts_if signer::address_of(validator_operator_account)
            != get_operator(validator_addr) with errors::INVALID_ARGUMENT;
    ...
    public fun is_valid(addr: address): bool acquires ValidatorConfig {
        exists<ValidatorConfig>(addr)
            && option::is_some(&borrow_global<ValidatorConfig>(addr).config)
    }
    ...
    spec module {
        invariant update forall a: address where old(exists<ValidatorConfig>(a))
            && exists<ValidatorConfig>(a):
            old(global<ValidatorConfig>(a).operator_account)
                != global<ValidatorConfig>(a).operator_account ==>
    ...
    spec module {
        ...
        invariant update forall validator: address where old(is_valid(validator)):
            is_valid(validator);
    }
