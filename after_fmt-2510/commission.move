...
module staking::commission {
    ...
    fun init_module(commission_signer: &signer) {
        let signer_cap =
            resource_account::retrieve_resource_account_cap(commission_signer, @manager);
        move_to(
            commission_signer,
            CommissionConfig {
                manager: @manager,
    ...
    public entry fun set_yearly_commission_amount(
        manager: &signer, new_commission: u64
    ...
    public entry fun distribute_commission(account: &signer) acquires CommissionConfig {
        ...
        assert!(
            balance >= MIN_BALANCE_FOR_DISTRIBUTION,
            EINSUFFICIENT_BALANCE_FOR_DISTRIBUTION
        );
    ...
    inline fun assert_manager(account: &signer) {
        assert!(
            signer::address_of(account) == CommissionConfig[@staking].manager,
            EUNAUTHORIZED
        );
    }
    ...
    inline fun assert_manager_or_operator(account: &signer) {
        ...
        assert!(
            account_addr == config.manager || account_addr == config.operator,
            EUNAUTHORIZED
        );
    }
