module admin::transaction_context_test {
    ...
    public entry fun store_secondary_signers_from_native_txn_context(
        _s: &signer
    ) acquires TransactionContextStore {
        let store = borrow_global_mut<TransactionContextStore>(@admin);
    ...
    public entry fun store_max_gas_amount_from_native_txn_context(
        _s: &signer
    ) acquires TransactionContextStore {
        let store = borrow_global_mut<TransactionContextStore>(@admin);
    ...
    public entry fun store_gas_unit_price_from_native_txn_context(
        _s: &signer
    ) acquires TransactionContextStore {
        let store = borrow_global_mut<TransactionContextStore>(@admin);
    ...
    entry fun store_entry_function_payload_from_native_txn_context<T1, T2, T3>(
        ...
        if (option::is_some(&payload_opt)) {
            ...
            assert!(
                option::some(option::destroy_some(payload_opt))
    ...
    entry fun store_multisig_payload_from_native_txn_context(
        ...
        if (option::is_some(&multisig_opt)) {
            ...
            assert!(
                option::some(option::destroy_some(multisig_opt))
    ...
    entry fun prepare_multisig_payload_test(
        s: &signer, payload: vector<u8>
    ) acquires TransactionContextStore {
        let store = borrow_global_mut<TransactionContextStore>(@admin);

        let multisig_account =
            multisig_account::get_next_multisig_account_address(signer::address_of(s));
    ...
    public entry fun store_monotonically_increasing_counter_single(
        _s: &signer
    ) acquires TransactionContextStore {
        let store = borrow_global_mut<TransactionContextStore>(@admin);
    ...
    public entry fun store_monotonically_increasing_counter_multiple(
        _s: &signer
    ) acquires TransactionContextStore {
        let store = borrow_global_mut<TransactionContextStore>(@admin);
    ...
    public entry fun test_monotonically_increasing_counter_format(
        _s: &signer
    ) acquires TransactionContextStore {
        let store = borrow_global_mut<TransactionContextStore>(@admin);
