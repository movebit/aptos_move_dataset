module aptos_std::iterable_table {
    ...
    public fun destroy_empty<K: copy + store + drop, V: store>(
    ...
    public fun remove<K: copy + store + drop, V: store>(
        table: &mut IterableTable<K, V>, key: K
    ): V {
    ...
    /// Aborts if there is no entry for `key`.
    public fun borrow<K: copy + store + drop, V: store>(
        table: &IterableTable<K, V>, key: K
    ): &V {
    ...
    /// Aborts if there is no entry for `key`.
    public fun borrow_mut<K: copy + store + drop, V: store>(
        table: &mut IterableTable<K, V>, key: K
    ): &mut V {
    ...
    public fun length<K: copy + store + drop, V: store>(
        table: &IterableTable<K, V>
    ): u64 {
    ...
    public fun empty<K: copy + store + drop, V: store>(
        table: &IterableTable<K, V>
    ): bool {
    ...
    public fun contains<K: copy + store + drop, V: store>(
        table: &IterableTable<K, V>, key: K
    ): bool {
    ...
    public fun head_key<K: copy + store + drop, V: store>(
        table: &IterableTable<K, V>
    ): Option<K> {
        table.head
    }
    ...
    public fun tail_key<K: copy + store + drop, V: store>(
        table: &IterableTable<K, V>
    ): Option<K> {
        table.tail
    }
