...
module aptos_framework::ethereum_derivable_account {
    ...
    fun construct_message(
        ...
        message.append(b"\nChain ID: ");
        message.append(
            *string_utils::to_string(&chain_id::get()).bytes()
        );
        message.append(b"\nNonce: ");
    ...
    fun authenticate_auth_data(
        ...
        let public_key_without_prefix =
            public_key_bytes.slice(1, public_key_bytes.length());
        ...
        let ethereum_address_without_prefix =
            abstract_public_key.ethereum_address.slice(
                2, abstract_public_key.ethereum_address.length()
            );
