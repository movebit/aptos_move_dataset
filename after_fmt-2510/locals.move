...
module 0xABCD::locals {
    ...
    struct NestedData<A: copy + drop, B: copy + drop, C: copy + drop, D: copy + drop, E: copy
        + drop, F: copy + drop> has copy, drop {
        abc: Data<A, B, C>,
        def: Data<D, E, F>
    }
    ...
    fun work_with_many_locals() {
        ...
        let c = vector[0_u8];
        let d = 120;
        let data1 = Data { a, b, c, d };
        let a = vector[0_u8];
        let b = vector[vector[120_u32]];
        let c = false;
        let d = 30;
        let data2 = Data { a, b, c, d };
        let data3 = data1;
        let data4 = data2;
        let data5 = NestedData { abc: data3, def: data4 };
        let data6 = NestedData { abc: data4, def: data3 };
        let data7 = NestedData { abc: data4, def: data4 };
        let data8 = NestedData { abc: data3, def: data3 };
        assert!(data7.abc.d == data5.def.d, 777);
        assert!(data8.abc.d == data6.def.d, 888);
    }

    fun generic_work_with_many_locals<A: copy + drop, B: copy + drop, C: copy + drop, D: copy
        + drop, E: copy + drop, F: copy + drop>() {
        let a = vector[];
        let b = vector[vector[]];
        let c = vector[false];
        let d = 40;
        let data1 = Data<A, B, vector<bool>> { a, b, c, d };
        let a = vector[];
        let b = vector[vector[]];
        let c = data1;
        let d = 23;
        let data2 = Data<D, E, Data<A, B, vector<bool>>> { a, b, c, d };
        let data3 = data1;
        let data4 = data2;
        let data5 = NestedData { abc: data3, def: data4 };
        let data6 = NestedData { abc: data4, def: data3 };
