...
module 0x42::TestSome {
    ...
    fun test_same_choice_same_args_via_spec_fun(n: u64): bool {
        n == 0
    }
    ...
    fun test_same_choice_different_args_via_spec_fun(x: u64, y: u64): bool {
        x == y
    }
    ...
    public fun new_ballot_id(counter: u64): BallotID {
        BallotID { counter }
    }
    ...
    spec module {
        invariant<Proposal> forall ballot_address: address where exists<Ballots<Proposal>>(
            ballot_address
    ...
    spec create_ballot {
        ensures !(
            get_ballot<Proposal>(signer::address_of(ballot_account), result).expiration_timestamp_secs
            <= 0
        );
    }
    ...
    spec get_ballots {
        let index = choose min i in range(ballots.ballots) where ballots.ballots[i].ballot_id
            .counter == 0;
        ensures true;
    }
}
