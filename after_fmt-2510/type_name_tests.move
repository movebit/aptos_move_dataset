    ...
    fun test_structs() {
        assert!(
            into_string(get<TestStruct>())
                == string(b"0xa::type_name_tests::TestStruct"),
            0
        );
        assert!(
            into_string(get<std::ascii::String>()) == string(b"0x1::ascii::String"),
            0
        );
        assert!(
            into_string(get<std::option::Option<u64>>())
                == string(b"0x1::option::Option<u64>"),
            0
        );
        assert!(
            into_string(get<std::string::String>()) == string(b"0x1::string::String"),
            0
        );
    }
    ...
    fun test_generics() {
        assert!(
            into_string(get<TestGenerics<std::string::String>>())
                == string(b"0xa::type_name_tests::TestGenerics<0x1::string::String>"),
            0
        );
        assert!(
            into_string(get<vector<TestGenerics<u64>>>())
                == string(b"vector<0xa::type_name_tests::TestGenerics<u64>>"),
            0
        );
        assert!(
            into_string(
