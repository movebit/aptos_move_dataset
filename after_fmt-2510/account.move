module CoreFramework::Account {
    ...
    public fun initialize<T>(
        ...
        assert!(
            signer::address_of(account) == @CoreResources,
            errors::requires_address(ENOT_CORE_FRAMEWORK)
        );
        move_to(account, Marker<T> {});
        move_to(
            account,
            ChainSpecificAccountInfo {
                module_addr,
                module_name,
    ...
    fun assert_is_marker<T>() {
        assert!(
            exists<Marker<T>>(@CoreResources),
            errors::invalid_argument(ENOT_MARKER_TYPE)
        )
    }
    ...
    public fun create_account<T>(
        new_address: address, authentication_key_prefix: vector<u8>, _witness: &T
    ): (signer, vector<u8>) {
        assert_is_marker<T>();
        // there cannot be an Account resource under new_addr already.
        assert!(
            !exists<Account>(new_address),
            errors::already_published(EACCOUNT)
        );

        let new_account = create_signer(new_address);
        let authentication_key =
            create_authentication_key(&new_account, authentication_key_prefix);
        move_to(
            &new_account,
            Account {
    ...
    public fun prologue(
        ...
        assert!(
            exists<Account>(transaction_sender),
    ...
    public fun writeset_epilogue<T>(
        account: &signer, should_trigger_reconfiguration: bool, witness: &T
    ) acquires Account {
