module 0xABCD::maps_example {
    ...
    inline fun do_test_add_remove(
        len: u64, repeats: u64, add: |u64|, remove: |u64|
    ) {
    ...
    public entry fun test_add_remove_simple_map(
        sender: &signer, len: u64, repeats: u64
    ) {
    ...
    public entry fun test_add_remove_ordered_map(
        sender: &signer, len: u64, repeats: u64
    ) {
    ...
    public entry fun test_add_remove_big_ordered_map(
        sender: &signer,
        len: u64,
        repeats: u64,
        ...
        let map =
            big_ordered_map::new_with_config(inner_max_degree, leaf_max_degree, false);
        do_test_add_remove(
            len,
            repeats,
            |x| {
