...
module coin_listing {
    ...
    public entry fun init_fixed_price<CoinType>(
        ...
        init_fixed_price_internal<CoinType>(
            seller, object, fee_schedule, start_time, price
        );
    }
    ...
    public(friend) fun init_fixed_price_internal<CoinType>(
        ...
        let (listing_signer, constructor_ref) =
            init<CoinType>(seller, object, fee_schedule, start_time, price);

        let fixed_price_listing = FixedPriceListing<CoinType> { price };
    ...
    public(friend) fun init_fixed_price_for_tokenv1_internal<CoinType>(
        ...
        let object =
            listing::create_tokenv1_container(
                seller,
                token_creator,
                token_collection,
                token_name,
    ...
    public entry fun purchase<CoinType>(
        purchaser: &signer, object: Object<Listing>
        ...
        let (price, type) =
            if (exists<AuctionListing<CoinType>>(listing_addr)) {
                let AuctionListing {
                    starting_bid: _,
                    bid_increment: _,
                    current_bid,
                ...
                assert!(
                    option::is_some(&buy_it_now_price),
                    error::invalid_argument(ENO_BUY_IT_NOW)
                );
    ...
    public entry fun end_fixed_price<CoinType>(
        seller: &signer, object: Object<Listing>
    ...
    public entry fun bid<CoinType>(
        bidder: &signer, object: Object<Listing>, bid_amount: u64
        ...
        let now = timestamp::now_seconds();
        assert!(
            now < auction_listing.auction_end_time,
            error::invalid_state(EAUCTION_ENDED)
        );
        ...
        let (previous_bidder, previous_bid, minimum_bid) =
            if (option::is_some(&auction_listing.current_bid)) {
                ...
                (
                    option::some(bidder),
                    option::some(current_bid),
                    current_bid + auction_listing.bid_increment
                )
            } else {
        ...
        let bid = Bid { bidder: signer::address_of(bidder), coins };
    ...
    public entry fun complete_auction<CoinType>(
        completer: &signer, object: Object<Listing>
    ) acquires AuctionListing {
        let listing_addr = listing::assert_started(&object);
        assert!(
            exists<AuctionListing<CoinType>>(listing_addr),
    ...
    inline fun complete_purchase<CoinType>(
        ...
        let actual_commission_charge = math64::min(
            coin::value(&coins), commission_charge
        );
...
module listing_tests {
    ...
    use marketplace::test_utils::{
        mint_tokenv2_with_collection_royalty,
        mint_tokenv1_additional_royalty,
        mint_tokenv1
    };
    ...
    fun test_fixed_price(
        ...
        assert!(
            coin_listing::price<AptosCoin>(listing) == option::some(500),
            0
        );
    ...
    fun test_auction_purchase(
        ...
        assert!(
            coin_listing::current_amount<AptosCoin>(listing) == option::none(),
            0
        );
        assert!(
            coin_listing::current_bidder<AptosCoin>(listing) == option::none(),
            0
        );
    ...
    fun test_auction_bid_then_purchase(
        ...
        assert!(
            coin_listing::current_amount<AptosCoin>(listing) == option::some(100),
            0
        );
        assert!(
        ...
        assert!(
            coin_listing::current_amount<AptosCoin>(listing) == option::some(150),
            0
        );
        assert!(
            coin_listing::current_bidder<AptosCoin>(listing)
    ...
    fun test_not_started_fixed_price(
        ...
        let listing =
            coin_listing::init_fixed_price_internal<AptosCoin>(
                seller,
    ...
    fun test_not_started_auction(
        ...
        let fee_schedule = test_utils::fee_schedule(marketplace);
        let listing =
            coin_listing::init_auction_internal<AptosCoin>(
                seller,
                object::convert(token),
                fee_schedule,
                timestamp::now_seconds() + 1,
    ...
    inline fun fixed_price_listing_with_token(
        marketplace: &signer, seller: &signer, token: Object<Token>
    ): (Object<Token>, Object<FeeSchedule>, Object<Listing>) {
        let fee_schedule = test_utils::fee_schedule(marketplace);
        let listing =
            coin_listing::init_fixed_price_internal<AptosCoin>(
    ...
    fun test_fixed_price_for_token_v1_high_royalty(
        ...
        let (_fee_schedule, listing) =
            fixed_price_listing_for_tokenv1_with_token(marketplace, seller, &token_id);
        coin_listing::purchase<AptosCoin>(purchaser, listing);
        assert!(tokenv1::balance_of(purchaser_addr, token_id) == 1, 0);
        // TODO balance checks
    }
