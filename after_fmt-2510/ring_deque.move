...
module ring_deque::ring_deque {
    ...
    public fun capacity<T>(rd_ref: &RingDeque<T>): u64 {
        rd_ref.capacity
    }

    public fun length<T>(rd_ref: &RingDeque<T>): u64 {
        rd_ref.length
    }

    public fun is_empty<T>(rd_ref: &RingDeque<T>): bool {
        rd_ref.length == 0
    }
    ...
    public fun borrow_front_mut<T>(rd_ref_mut: &mut RingDeque<T>): &mut T {
        ...
        option::borrow_mut(vector::borrow_mut(&mut rd_ref_mut.data, rd_ref_mut.front))
    }
    ...
    public fun borrow_back_mut<T>(rd_ref_mut: &mut RingDeque<T>): &mut T {
        assert!(rd_ref_mut.length > 0, E_EMPTY);
        option::borrow_mut(vector::borrow_mut(&mut rd_ref_mut.data, rd_ref_mut.back))
    }
    ...
    public fun push_back<T>(rd_ref_mut: &mut RingDeque<T>, value: T) {
        ...
        if (rd_ref_mut.length > 0) {
            let max_index = rd_ref_mut.capacity - 1;
            rd_ref_mut.back =
                if (rd_ref_mut.back == max_index) { 0 }
                else {
                    rd_ref_mut.back + 1
                };
        };
    ...
    public fun pop_front<T>(rd_ref_mut: &mut RingDeque<T>): T {
        assert!(rd_ref_mut.length > 0, E_EMPTY);
        let val =
            option::extract(vector::borrow_mut(&mut rd_ref_mut.data, rd_ref_mut.front));
        if (rd_ref_mut.length > 1) {
            let max_index = rd_ref_mut.capacity - 1;
            rd_ref_mut.front =
                if (rd_ref_mut.front == max_index) { 0 }
                else {
                    rd_ref_mut.front + 1
                };
        };
    ...
    public fun pop_back<T>(rd_ref_mut: &mut RingDeque<T>): T {
        assert!(rd_ref_mut.length > 0, E_EMPTY);
        let val =
            option::extract(vector::borrow_mut(&mut rd_ref_mut.data, rd_ref_mut.back));
        if (rd_ref_mut.length > 1) {
            let max_index = rd_ref_mut.capacity - 1;
            rd_ref_mut.back =
                if (rd_ref_mut.back == 0) {
                    max_index
                } else {
                    rd_ref_mut.back - 1
                };
