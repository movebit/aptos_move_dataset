...
module aptos_framework::multisig_account {
    ...
    public fun is_owner(owner: address, multisig_account: address): bool acquires MultisigAccount {
        vector::contains(
            &borrow_global<MultisigAccount>(multisig_account).owners, &owner
        )
    }
    ...
    #[test]
    public entry fun test_create_multisig_account_on_top_of_existing_with_signer() acquires MultisigAccount {
        setup();
    ...
    #[test]
    public entry fun test_create_multisig_account_on_top_of_existing_and_revoke_auth_key_with_signer() acquires MultisigAccount {
        setup();
    ...
    public entry fun test_update_metadata(owner: &signer) acquires MultisigAccount {
        ...
        assert!(
            simple_map::borrow(&updated_metadata, &utf8(b"key1")) == &vector[1],
            0
        );
        assert!(
            simple_map::borrow(&updated_metadata, &utf8(b"key2")) == &vector[2],
            0
        );
    }
