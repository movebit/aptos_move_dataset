...
module DiemFramework::DualAttestation {
    ...
    public(friend) fun publish_credential(
        ...
        move_to(
            ...
            Credential {
                ...
                compliance_key_rotation_events: event::new_event_handle<
    ...
    public fun rotate_base_url(account: &signer, new_url: vector<u8>) acquires Credential {
        ...
        assert!(
            exists<Credential>(addr),
            errors::not_published(ECREDENTIAL)
        );
    ...
    spec schema AssertSignatureValidAbortsIf {
        ...
        aborts_if !spec_signature_is_valid(
            payer,
            payee,
            metadata_signature,
            metadata,
    ...
    spec schema AssertPaymentOkAbortsIf<Currency> {
        ...
        metadata: vector<u8>;
        metadata_signature: vector<u8>;
        include len(metadata_signature) == 0 ==>
            DualAttestationRequiredAbortsIf<Currency> { deposit_value: value };
        include (
    ...
    public fun initialize(dr_account: &signer) {
        ...
        assert!(
            ...
            errors::already_published(ELIMIT)
        );
        let initial_limit =
    ...
    spec initialize {
        ...
        let initial_limit = INITIAL_DUAL_ATTESTATION_LIMIT
            * Diem::spec_scaling_factor<XDX>();
    ...
    public fun set_microdiem_limit(
        tc_account: &signer, micro_xdx_limit: u64
    ) acquires Limit {
