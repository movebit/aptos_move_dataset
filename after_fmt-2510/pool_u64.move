...
module aptos_std::pool_u64 {
    ...
    public fun buy_in(
        self: &mut Pool, shareholder: address, coins_amount: u64
    ): u64 {
        ...
        assert!(
            MAX_U64 - self.total_shares >= new_shares,
            error::invalid_argument(EPOOL_TOTAL_COINS_OVERFLOW)
        );
    ...
    public fun redeem_shares(
        ...
        assert!(
            self.contains(shareholder),
            error::invalid_argument(ESHAREHOLDER_NOT_FOUND)
        );
        assert!(
            self.shares(shareholder) >= shares_to_redeem,
    ...
    public fun transfer_shares(
        ...
        assert!(
            self.contains(shareholder_1),
            error::invalid_argument(ESHAREHOLDER_NOT_FOUND)
        );
        assert!(
            self.shares(shareholder_1) >= shares_to_transfer,
            error::invalid_argument(EINSUFFICIENT_SHARES)
        );
    ...
    fun deduct_shares(
        ...
        assert!(
            self.shares(shareholder) >= num_shares,
            error::invalid_argument(EINSUFFICIENT_SHARES)
        );
    ...
    public fun shares_to_amount_with_total_coins(
        ...
        if (self.total_coins == 0 || self.total_shares == 0) { 0 }
        else {
            // Shares price = total_coins / total existing shares.
