...
module defi::reserve {
    ...
    spec Coin1Info {
        ...
        invariant fixed_point32::spec_multiply_u64(
            total_value, reserve_coin2.backing_ratio
    ...
    acquires Coin1Info {
        ...
        let coin2_amount_to_reserve =
            fixed_point32::multiply_u64(
                amount_to_mint, *&coin1info.reserve_coin2.backing_ratio
            ) + 1;
        assert!(backing_coin2 == coin2_amount_to_reserve, 2);
        coin1info.reserve_coin2.backing_value =
            coin1info.reserve_coin2.backing_value + coin2_amount_to_reserve;
    ...
    spec mint_coin1 {
        ...
        let coin2_amount_to_reserve = fixed_point32::spec_multiply_u64(
            amount_to_mint, coin1info.reserve_coin2.backing_ratio
        ) + 1;
    ...
    public fun mint_coin1_incorrect(
        amount_to_mint: u64, backing_coin2: u64
    ): u64 // returns the minted Coin1.
    acquires Coin1Info {
        assert!(amount_to_mint > 0, 1);
    ...
    acquires Coin1Info {
        ...
        let coin2_amount_to_return =
            fixed_point32::multiply_u64(
                amount_to_burn, *&coin1info.reserve_coin2.backing_ratio
            );
        assert!(coin1info.reserve_coin2.backing_value >= coin2_amount_to_return, 1);
        coin1info.reserve_coin2.backing_value =
            coin1info.reserve_coin2.backing_value - coin2_amount_to_return;
    ...
    spec module {
        ...
        invariant exists<Coin1Info>(ADMIN) ==>
            global<Coin1Info>(ADMIN).reserve_coin2.backing_ratio
                == fixed_point32::spec_create_from_rational(1, 5);
    }
}
