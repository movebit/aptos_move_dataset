module aptos_std::smart_vector {
    ...
    public fun append<T: store>(
        self: &mut SmartVector<T>, other: SmartVector<T>
    ) {
    ...
    public fun push_back<T: store>(self: &mut SmartVector<T>, val: T) {
        ...
        if (len == inline_len) {
            let bucket_size =
                if (self.inline_capacity.is_some()) {
                    ...
                    *self.bucket_size.borrow()
                    ...
                    let estimated_avg_size =
                        max(
                            (size_of_val(&self.inline_vec) + val_size) / (inline_len + 1),
                            1
                        );
    ...
    public fun length<T>(self: &SmartVector<T>): u64 {
        self.inline_vec.length()
            + if (self.big_vec.is_none()) { 0 }
            else {
    ...
    public inline fun enumerate_mut<T>(
        self: &mut SmartVector<T>, f: |u64, &mut T|
    ) {
        let len = self.length();
        for (i in 0..len) {
    ...
    fun smart_vector_reverse_test() {
        ...
        while (i + 1 < len) {
            assert!(
                *v.big_vec.borrow().borrow(i)
                    == *v.big_vec.borrow().borrow(i + 1) + 1,
                0
            );
            i += 1;
        };
