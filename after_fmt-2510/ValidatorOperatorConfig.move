...
module DiemFramework::ValidatorOperatorConfig {
    ...
    public(friend) fun publish(
        validator_operator_account: &signer, dr_account: &signer, human_name: vector<u8>
    ) {
    ...
    spec schema PublishAbortsIf {
        ...
        include Roles::AbortsIfNotDiemRoot { account: dr_account };
        include Roles::AbortsIfNotValidatorOperator { account: validator_operator_account };
        aborts_if has_validator_operator_config(validator_operator_addr) with errors::ALREADY_PUBLISHED;
    }
    ...
    public fun has_validator_operator_config(
        validator_operator_addr: address
    ): bool {
    ...
    /// # Persistence
    spec module {
        invariant update forall addr: address where old(
