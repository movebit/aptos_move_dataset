module tic_tac_toe::ttt {
    ...
    */
    ...
    public entry fun start_game(creator: &signer) {
        ...
        assert!(
            !exists<Game>(signer::address_of(creator)),
            error::already_exists(EGAME_ALREADY_EXISTS_FOR_USER)
        );
    ...
    */
    public entry fun join_as_player_o(
        new_user: &signer, game_addr: address
    ) acquires Game {
        let new_user_addr = signer::address_of(new_user);
        assert!(
            new_user_addr != game_addr,
            error::invalid_argument(ECANNOT_JOIN_AS_TWO_PLAYERS)
        );

        assert!(
            exists<Game>(game_addr),
            error::not_found(EGAME_DOESNT_EXIST)
        );
    ...
    */
    ...
    fun test_join_as_player_o(
        creator: &signer, new_user: &signer, game_addr: address
    ) acquires Game {
        create_account_for_test(signer::address_of(creator));
        start_game(creator);
    ...
    #[test(
        creator = @0x123, new_user = @0x223, third_user = @0x323, game_addr = @0x123
    )]
    #[expected_failure]
    ...
    fun test_valid_row_win_1(
        user1_signer: &signer, user1: address, user2: address
    ) {
    ...
    #[test(user1_signer = @0x123, user1 = @0x123, user2 = @0x223)]
    #[expected_failure]
    fun test_place_move_after_game_win(
        user1_signer: &signer, user1: address, user2: address
    ) {
        create_account_for_test(signer::address_of(user1_signer));
        let game = initalize_game(user1_signer);
