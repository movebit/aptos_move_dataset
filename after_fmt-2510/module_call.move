...
module M {
    ...
    fun t2() {
        let () = X::bing(X::baz(X::bar(X::foo()))); // invalid
        let () = X::bing(X::baz(X::bar(X::foo()))); // invalid
        let () = X::bing(X::baz(X::bar(1))); // invalid
        let () = X::bing(X::baz(@0x0, 1)); // invalid
        let () = X::bing(false, @0x0, 1);
    }

    fun t3() {
        let () = bing(baz(bar(foo()))); // invalid
        let () = bing(baz(bar(foo()))); // invalid
        let () = bing(baz(bar(1))); // invalid
        let () = bing(baz(@0x0, 1)); // invalid
