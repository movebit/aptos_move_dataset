...
module aptos_framework::stake {
            ...
            spec {
                ...
                invariant (
                    ...
                    (
                        ...
                        == ghost_valid_perf.validators[option::borrow(ghost_proposer_idx)]
    ...
    public(friend) fun on_new_epoch() acquires AptosCoinCapabilities, PendingTransactionFee, StakePool, TransactionFeeConfig, ValidatorConfig, ValidatorPerformance, ValidatorSet {
        ...
        while ({
            ...
            vector::push_back(
                ...
                IndividualValidatorPerformance { successful_proposals: 0, failed_proposals:
                    0 }
            );
        ...
        if (exists<PendingTransactionFee>(@aptos_framework)) {
            ...
            validator_set.active_validators.for_each_ref(
                |v| pending_fee_by_validator.add(
        ...
        while ({
            ...
            let candidate =
                if (candidate_idx < num_cur_actives) {
                    ...
                    vector::borrow(
                        &cur_validator_set.pending_active, candidate_idx
                            - num_cur_actives
                    )
                };
    ...
    fun validator_consensus_infos_from_validator_set(
        ...
        while ({
            ...
            vector::push_back(
                &mut validator_consensus_infos,
                validator_consensus_info::default()
            );
            idx = idx + 1;
        };
        spec {
    ...
    public entry fun test_pending_active_validator_leaves_validator_set(
        ...
        assert!(
            borrow_global<ValidatorSet>(@aptos_framework).total_joining_power == 100,
            0
        );
        ...
        assert!(
            borrow_global<ValidatorSet>(@aptos_framework).total_joining_power == 0,
            1
        );
    }
    ...
    public entry fun test_multiple_validators_join_and_leave(
        ...
        assert!(
            vector::borrow(
                &borrow_global<ValidatorSet>(@aptos_framework).pending_active, 0
        ...
        end_epoch();
        assert!(
            get_validator_state(validator_1_address) == VALIDATOR_STATUS_INACTIVE,
            11
        );
    }
