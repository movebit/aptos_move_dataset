...
module aptos_framework::primary_fungible_store {
    ...
    public fun grant_apt_permission(
        master: &signer, permissioned: &signer, amount: u64
    ) {
        fungible_asset::grant_permission_by_address(
            master,
            permissioned,
            object::create_user_derived_object_address(
                signer::address_of(permissioned), @aptos_fungible_asset
            ),
            amount
        );
    }
    ...
    #[test(creator = @0xcafe, aaron = @0xface)]
    fun test_permissioned_flow(creator: &signer, aaron: &signer) acquires DeriveRefPod {
        let (creator_ref, metadata) = create_test_token(creator);
