...
module DiemFramework::Diem {
    ...
    public(friend) fun publish_burn_capability<CoinType>(
        tc_account: &signer, cap: BurnCapability<CoinType>
    ) {
    ...
    public fun mint<CoinType>(
        account: &signer, value: u64
    ): Diem<CoinType> acquires CurrencyInfo, MintCapability {
        let addr = signer::address_of(account);
        assert!(
            exists<MintCapability<CoinType>>(addr),
            errors::requires_capability(EMINT_CAPABILITY)
        );
    ...
    public fun burn<CoinType>(
        ...
        assert!(
            ...
            errors::requires_capability(EBURN_CAPABILITY)
        );
    ...
    public(friend) fun mint_with_capability<CoinType>(
        value: u64, _capability: &MintCapability<CoinType>
        ...
        assert!(
            MAX_U128 - info.total_value >= (value as u128),
    ...
    spec schema MintEnsures<CoinType> {
        ...
        ensures exists<CurrencyInfo<CoinType>>(@CurrencyInfo);
        ensures post_currency_info
            == update_field(currency_info, total_value, currency_info.total_value
                + value);
    ...
    fun preburn_with_resource<CoinType>(
        ...
        let info = borrow_global_mut<CurrencyInfo<CoinType>>(@CurrencyInfo);
        assert!(
            MAX_U64 - info.preburn_value >= coin_value,
    ...
    spec schema PreburnEnsures<CoinType> {
        ...
        ensures post_info
            == update_field(info, preburn_value, info.preburn_value + amount);
    }
    ...
    spec schema PreburnWithResourceEmits<CoinType> {
        ...
        let msg = PreburnEvent { amount, currency_code, preburn_address };
    ...
    public fun create_preburn<CoinType>(tc_account: &signer): Preburn<CoinType> {
        Roles::assert_treasury_compliance(tc_account);
        assert_is_currency<CoinType>();
    ...
    fun publish_preburn_queue<CoinType>(account: &signer) {
        ...
        move_to(
            account,
            PreburnQueue<CoinType> { preburns: vector::empty() }
        )
    }
    ...
    public(friend) fun publish_preburn_queue_to_account<CoinType>(
        account: &signer, tc_account: &signer
    ) acquires CurrencyInfo {
        Roles::assert_designated_dealer(account);
        Roles::assert_treasury_compliance(tc_account);
        assert!(
            !is_synthetic_currency<CoinType>(),
            errors::invalid_argument(EIS_SYNTHETIC_CURRENCY)
        );
    ...
    fun upgrade_preburn<CoinType>(account: &signer) acquires Preburn, PreburnQueue {
        Roles::assert_designated_dealer(account);
        ...
        if (preburn_exists && !preburn_queue_exists) {
            ...
            if (to_burn.value > 0) {
                add_preburn_to_queue(
                    account,
                    PreburnWithMetadata { preburn: Preburn { to_burn }, metadata: x"" }
                )
            } else {
    ...
    fun add_preburn_to_queue<CoinType>(
        account: &signer, preburn: PreburnWithMetadata<CoinType>
    ) acquires PreburnQueue {
        let account_addr = signer::address_of(account);
        assert!(
            exists<PreburnQueue<CoinType>>(account_addr),
            errors::invalid_state(EPREBURN_QUEUE)
        );
        assert!(value(&preburn.preburn.to_burn) > 0, errors::invalid_argument(EPREBURN));
        let preburns =
            &mut borrow_global_mut<PreburnQueue<CoinType>>(account_addr).preburns;
        assert!(
    ...
    spec schema AddPreburnToQueueAbortsIf<CoinType> {
        ...
        aborts_if exists<PreburnQueue<CoinType>>(account_addr)
            && vector::length(
                global<PreburnQueue<CoinType>>(account_addr).preburns
            ) >= MAX_OUTSTANDING_PREBURNS with errors::LIMIT_EXCEEDED;
    }
    ...
    public fun preburn_to<CoinType>(
        account: &signer, coin: Diem<CoinType>
    ...
    spec schema PreburnToAbortsIf<CoinType> {
        ...
        include UpgradePreburnAbortsIf<CoinType>;
        include AddPreburnToQueueAbortsIf<CoinType> {
            preburn: PreburnWithMetadata {
                preburn: spec_make_preburn(amount),
                metadata: x""
            }
        };
    }
    ...
    fun remove_preburn_from_queue<CoinType>(
        preburn_address: address, amount: u64
    ): PreburnWithMetadata<CoinType> acquires PreburnQueue {
        assert!(
            exists<PreburnQueue<CoinType>>(preburn_address),
            errors::not_published(EPREBURN_QUEUE)
        );
        // We search from the head of the queue
        let index = 0;
        let preburn_queue =
            &mut borrow_global_mut<PreburnQueue<CoinType>>(preburn_address).preburns;
        let queue_length = vector::length(preburn_queue);

        while ({
            spec {
                invariant index <= queue_length;
                invariant forall j in 0..index:
                    preburn_queue[j].preburn.to_burn.value != amount;
            };
            (index < queue_length)
        }) {
        ...
        spec {
            ...
            assert forall j in 0..queue_length:
                preburn_queue[j].preburn.to_burn.value != amount;
        };
    ...
    spec schema RemovePreburnFromQueueAbortsIf<CoinType> {
        ...
        aborts_if !exists<PreburnQueue<CoinType>>(preburn_address) with errors::NOT_PUBLISHED;
        aborts_if forall i in 0..len(preburn_queue):
            preburn_queue[i].preburn.to_burn.value != amount with errors::INVALID_STATE;
    }
    ...
    public(friend) fun burn_with_capability<CoinType>(
        ...
        let PreburnWithMetadata { preburn, metadata: _ } =
            remove_preburn_from_queue<CoinType>(preburn_address, amount);
    ...
    fun burn_with_resource_cap<CoinType>(
        ...
        if (!info.is_synthetic) {
            event::emit_event(
                ...
                BurnEvent { amount: value, currency_code, preburn_address }
            );
        };
    }
    ...
    spec schema BurnWithResourceCapEnsures<CoinType> {
        ...
        ensures spec_currency_info<CoinType>().preburn_value
            == old(spec_currency_info<CoinType>().preburn_value)
    ...
    fun cancel_burn_with_capability<CoinType>(
        ...
        let PreburnWithMetadata { preburn: Preburn { to_burn }, metadata: _ } =
            remove_preburn_from_queue<CoinType>(preburn_address, amount);
        ...
        if (!info.is_synthetic) {
            event::emit_event(
                ...
                CancelBurnEvent { amount, currency_code, preburn_address }
            );
        };
    ...
    spec schema CancelBurnWithCapEmits<CoinType> {
        ...
        emits CancelBurnEvent { amount, currency_code, preburn_address } to handle if !info
            .is_synthetic;
    }
    ...
    spec burn_now {
        ...
        include BurnWithResourceCapEmits<CoinType> {
            preburn: Preburn<CoinType> { to_burn: coin }
        };
        ensures preburn.to_burn.value == 0;
        ensures post_info
            == update_field(info, total_value, info.total_value - coin.value);
    }
    ...
    public(friend) fun remove_burn_capability<CoinType>(
        account: &signer
    ): BurnCapability<CoinType> acquires BurnCapability {
        let addr = signer::address_of(account);
        assert!(
            exists<BurnCapability<CoinType>>(addr),
            errors::requires_capability(EBURN_CAPABILITY)
    ...
    public fun join<CoinType>(
        coin1: Diem<CoinType>, coin2: Diem<CoinType>
    ): Diem<CoinType> {
    ...
    public fun deposit<CoinType>(
        coin: &mut Diem<CoinType>, check: Diem<CoinType>
    ) {
    ...
    public fun register_currency<CoinType>(
        ...
        assert!(
            0 < scaling_factor && scaling_factor <= MAX_SCALING_FACTOR,
            errors::invalid_argument(ECURRENCY_INFO)
        );
        move_to(
            dr_account,
            CurrencyInfo<CoinType> {
                ...
                exchange_rate_update_events: event::new_event_handle<
                    ToXDXExchangeRateUpdateEvent>(dr_account)
            }
        );
    ...
    spec schema RegisterCurrencyAbortsIf<CoinType> {
        ...
        aborts_if exists<CurrencyInfo<CoinType>>(signer::address_of(dr_account)) with errors::ALREADY_PUBLISHED;
    ...
    public fun market_cap<CoinType>(): u128 acquires CurrencyInfo {
        assert_is_currency<CoinType>();
    ...
    /// rate is needed.
    public fun approx_xdx_for_value<FromCoinType>(from_value: u64): u64 acquires CurrencyInfo {
    ...
    spec schema ApproxXdmForValueAbortsIf<CoinType> {
        ...
        include fixed_point32::MultiplyAbortsIf {
            val: from_value,
            multiplier: xdx_exchange_rate
        };
    }
    ...
    /// rate is needed.
    public fun approx_xdx_for_coin<FromCoinType>(
        coin: &Diem<FromCoinType>
    ): u64 acquires CurrencyInfo {
    ...
    public fun is_synthetic_currency<CoinType>(): bool acquires CurrencyInfo {
    ...
    /// in its `CurrencyInfo`.
    public fun scaling_factor<CoinType>(): u64 acquires CurrencyInfo {
        assert_is_currency<CoinType>();
    ...
    public fun fractional_part<CoinType>(): u64 acquires CurrencyInfo {
        assert_is_currency<CoinType>();
    ...
    public fun currency_code<CoinType>(): vector<u8> acquires CurrencyInfo {
        assert_is_currency<CoinType>();
    ...
    public fun update_xdx_exchange_rate<FromCoinType>(
        tc_account: &signer, xdx_exchange_rate: FixedPoint32
    ) acquires CurrencyInfo {
        Roles::assert_treasury_compliance(tc_account);
        assert_is_currency<FromCoinType>();
        // XDX is not allowed to update the exchange rate.
        assert!(
            currency_code<FromCoinType>() != b"XDX",
            errors::invalid_argument(ECOIN)
        );
        ...
        event::emit_event(
            ...
            ToXDXExchangeRateUpdateEvent {
                ...
                new_to_xdx_exchange_rate: fixed_point32::get_raw_value(
                    *&currency_info.to_xdx_exchange_rate
                )
            }
        );
    }
    ...
    spec schema UpdateXDXExchangeRateEnsures<FromCoinType> {
        ...
        ensures spec_currency_info<FromCoinType>().to_xdx_exchange_rate
            == xdx_exchange_rate;
    }
    ...
    public fun xdx_exchange_rate<CoinType>(): FixedPoint32 acquires CurrencyInfo {
        assert_is_currency<CoinType>();
    ...
    public fun update_minting_ability<CoinType>(
        tc_account: &signer, can_mint: bool
    ) acquires CurrencyInfo {
        Roles::assert_treasury_compliance(tc_account);
        assert_is_currency<CoinType>();
    ...
    public fun assert_is_currency<CoinType>() {
        assert!(
            is_currency<CoinType>(),
            errors::not_published(ECURRENCY_INFO)
        );
    }
    ...
    public fun assert_is_SCS_currency<CoinType>() acquires CurrencyInfo {
        ...
        assert!(
            ...
            errors::invalid_state(ECURRENCY_INFO)
        );
    }
    ...
    spec module {
        ...
        apply TotalValueNotIncrease<CoinType> to *<CoinType> except mint<CoinType>, mint_with_capability<CoinType>;
        ...
        invariant<CoinType> forall mint_cap_owner: address where exists<MintCapability<CoinType>>(
            mint_cap_owner
        ...
        invariant<CoinType> is_SCS_currency<CoinType>() ==>
            (
                forall mint_cap_owner1: address,
                mint_cap_owner2: address where(
        ...
        invariant<CoinType> forall addr3: address where spec_has_mint_capability<CoinType>(
            addr3
        ): is_SCS_currency<CoinType>();
    }
    ...
    spec schema TotalValueNotIncrease<CoinType> {
        ...
        ensures old(spec_is_currency<CoinType>()) ==>
            spec_currency_info<CoinType>().total_value
                <= old(spec_currency_info<CoinType>().total_value);
    }
    ...
    spec module {
        ...
        apply TotalValueNotDecrease<CoinType> to *<CoinType> except burn<CoinType>, burn_with_capability<CoinType>, burn_with_resource_cap<CoinType>, burn_now<CoinType>;
        ...
        invariant<CoinType> forall addr1: address:
    ...
    spec schema PreburnValueNotIncrease<CoinType> {
        ...
        ensures old(spec_is_currency<CoinType>()) ==>
            spec_currency_info<CoinType>().preburn_value
                <= old(spec_currency_info<CoinType>().preburn_value);
    }
    ...
    spec module {
        ...
        apply PreburnValueNotDecrease<CoinType> to *<CoinType> except burn<CoinType>, burn_with_capability<CoinType>, burn_with_resource_cap<CoinType>, burn_now<CoinType>, cancel_burn<CoinType>, cancel_burn_with_capability<CoinType>;

        /// Only preburn functions can increase the preburn value of currency [[H4]][PERMISSION].
        apply PreburnValueNotIncrease<CoinType> to *<CoinType> except preburn_to<CoinType>, preburn_with_resource<CoinType>;
        ...
        apply PreservePreburnQueueAbsence<CoinType> to *<CoinType> except publish_preburn_queue<CoinType>, publish_preburn_queue_to_account<CoinType>;
        ...
        invariant<CoinType> forall dd_addr: address where exists<Preburn<CoinType>>(
            dd_addr
        ):
        ...
        /// the same address.
        invariant<CoinType> forall dd_addr: address where exists<PreburnQueue<CoinType>>(
            dd_addr
        ): !exists<Preburn<CoinType>>(dd_addr);

        /// A `Preburn` resource can only be published holding a currency type.
        invariant<CoinType> forall addr: address where exists<Preburn<CoinType>>(addr):
        ...
        invariant<CoinType> update forall dr_addr: address where old(
            exists<CurrencyInfo<CoinType>>(dr_addr)
        ): exists<CurrencyInfo<CoinType>>(dr_addr);

        /// resource struct `PreburnQueue<CoinType>` is persistent
        invariant<CoinType> update forall tc_addr: address where old(
    ...
    spec module {
        ...
        invariant<CoinType> forall a: address where spec_has_mint_capability<CoinType>(a): is_currency<CoinType>();

        /// If an address has a burn capability, it is a registered currency.
        invariant<CoinType> forall a: address where spec_has_burn_capability<CoinType>(a): is_currency<CoinType>();
    }
    ...
    spec module {
        ...
        fun spec_signed_by_preburn_owner<CoinType>(): bool {
            (
                exists a: address:
                    signer::is_txn_signer_addr(a) && spec_has_preburn<CoinType>(a)
            )
        }
    }
}
