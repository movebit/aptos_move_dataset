...
module DiemFramework::DiemVMConfig {
    ...
    public fun set_gas_constants(
        ...
        gas_constants.global_memory_per_byte_write_cost =
    ...
    spec set_gas_constants {
        ...
        ensures DiemConfig::get<DiemVMConfig>()
            == DiemVMConfig {
                gas_schedule: GasSchedule {
    ...
    /// The permission "UpdateVMConfig" is granted to DiemRoot [[H11]][PERMISSION].
    spec module {
        invariant [suspendable] forall addr: address where exists<DiemConfig<DiemVMConfig>>(
            addr
        ): addr == @DiemRoot;
