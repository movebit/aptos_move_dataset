...
module DiemFramework::DiemVersion {
    ...
    spec initialize {
        ...
        include DiemConfig::PublishNewConfigEnsures<DiemVersion> {
            payload: DiemVersion { major: initial_version }
        };
    }
    ...
    public fun set(dr_account: &signer, major: u64) {
        ...
        DiemConfig::set<DiemVersion>(dr_account, DiemVersion { major });
    }
    ...
    spec module {
        invariant [suspendable] forall addr: address where exists<DiemConfig<DiemVersion>>(
            addr
        ): addr == @DiemRoot;

        invariant update[suspendable] old(DiemConfig::spec_is_published<DiemVersion>())
            && DiemConfig::spec_is_published<DiemVersion>()
            && old(DiemConfig::get<DiemVersion>().major)
                != DiemConfig::get<DiemVersion>().major ==>
    ...
    spec module {
        ...
        invariant update[suspendable] DiemTimestamp::is_operating() ==>
            (
                old(DiemConfig::get<DiemVersion>().major)
                    <= DiemConfig::get<DiemVersion>().major
            );
