...
module DiemFramework::DiemConfig {
    ...
    public fun get<Config: copy + drop + store>(): Config acquires DiemConfig {
        let addr = @DiemRoot;
        assert!(
            exists<DiemConfig<Config>>(addr),
            errors::not_published(EDIEM_CONFIG)
        );
    ...
    public(friend) fun set<Config: copy + drop + store>(
        account: &signer, payload: Config
        ...
        assert!(
            ...
            errors::requires_capability(EMODIFY_CAPABILITY)
        );
    ...
    spec schema AbortsIfNotModifiable<Config> {
        account: signer;
        aborts_if !exists<ModifyConfigCapability<Config>>(signer::address_of(account)) with errors::REQUIRES_CAPABILITY;
    }
    ...
    public(friend) fun set_with_capability_and_reconfigure<Config: copy + drop + store>(
        _cap: &ModifyConfigCapability<Config>, payload: Config
    ) acquires DiemConfig, Configuration {
        let addr = @DiemRoot;
        assert!(
            exists<DiemConfig<Config>>(addr),
    ...
    public(friend) fun publish_new_config_and_get_capability<Config: copy + drop + store>(
        dr_account: &signer, payload: Config
    ...
    public(friend) fun publish_new_config<Config: copy + drop + store>(
        ...
        let capability =
            publish_new_config_and_get_capability<Config>(dr_account, payload);
        assert!(
    ...
    fun reconfigure_() acquires Configuration {
        ...
        if (DiemTimestamp::is_genesis()
            || DiemTimestamp::now_microseconds() == 0
    ...
    spec reconfigure_ {
        ...
        ensures !(spec_reconfigure_omitted() || (config.last_reconfiguration_time
            == now)) ==>
            post_config
                == update_field(
                    update_field(config, epoch, config.epoch + 1),
    ...
    spec schema ReconfigureEmits {
        ...
        let post msg = NewEpochEvent { epoch: post_config.epoch };
    ...
    spec emit_genesis_reconfiguration_event {
        ...
        let post msg = NewEpochEvent { epoch: config.epoch };
    ...
    #[test_only]
    public fun set_for_testing<Config: copy + drop + store>(
        account: &signer, payload: Config
    ) acquires DiemConfig, Configuration {
        set(account, payload)
    }
    ...
    spec module {
        ...
        invariant<ConfigType> forall config_address: address where exists<DiemConfig<
            ConfigType>>(config_address): config_address == @DiemRoot;

        /// Published configurations are persistent.
        invariant<ConfigType> update old(spec_is_published<ConfigType>()) ==>
            spec_is_published<ConfigType>();

        /// If `ModifyConfigCapability<Config>` is published, it is persistent.
        invariant<ConfigType> update old(
