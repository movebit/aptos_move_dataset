...
module std::bn254_algebra {
    ...
    fun test_fq12(fx: signer) {
        ...
        let val_7_another =
            deserialize<Fq12, FormatFq12LscLsb>(&FQ12_VAL_7_SERIALIZED).extract();
        assert!(eq(&val_7, &val_7_another), 1);
        assert!(FQ12_VAL_7_SERIALIZED == serialize<Fq12, FormatFq12LscLsb>(&val_7), 1);
        assert!(
            deserialize<Fq12, FormatFq12LscLsb>(&x"ffff").is_none(),
            1
        );
        ...
        assert!(
            eq(
                &zero<Gt>(),
                &downcast<Fq12, Gt>(&val_1).extract()
            ),
            1
        );
        // upcasting
    ...
    fun test_g1affine(fx: signer) {
        ...
        let generator_from_uncomp =
            deserialize<G1, FormatG1Uncompr>(&G1_GENERATOR_SERIALIZED_UNCOMP).extract();
        ...
        let inf_from_uncomp =
            deserialize<G1, FormatG1Uncompr>(&G1_INF_SERIALIZED_UNCOMP).extract();
        let inf_from_comp =
            deserialize<G1, FormatG1Compr>(&G1_INF_SERIALIZED_COMP).extract();
        assert!(eq(&point_at_infinity, &inf_from_comp), 1);
        assert!(eq(&point_at_infinity, &inf_from_uncomp), 1);

        let point_7g_from_uncomp =
            deserialize<G1, FormatG1Uncompr>(&G1_GENERATOR_MUL_BY_7_SERIALIZED_UNCOMP).extract();
        let point_7g_from_comp =
            deserialize<G1, FormatG1Compr>(&G1_GENERATOR_MUL_BY_7_SERIALIZED_COMP).extract();
    ...
    fun test_g2affine(fx: signer) {
        ...
        let generator_from_uncomp =
            deserialize<G2, FormatG2Uncompr>(&G2_GENERATOR_SERIALIZED_UNCOMP).extract();
        let generator_from_comp =
            deserialize<G2, FormatG2Compr>(&G2_GENERATOR_SERIALIZED_COMP).extract();
        assert!(eq(&generator, &generator_from_comp), 1);
        assert!(eq(&generator, &generator_from_uncomp), 1);
        assert!(
        ...
        let inf_from_uncomp =
            deserialize<G2, FormatG2Uncompr>(&G2_INF_SERIALIZED_UNCOMP).extract();
        let inf_from_comp =
            deserialize<G2, FormatG2Compr>(&G2_INF_SERIALIZED_COMP).extract();
        assert!(eq(&point_at_infinity, &inf_from_comp), 1);
        assert!(eq(&point_at_infinity, &inf_from_uncomp), 1);
        let point_7g_from_uncomp =
            deserialize<G2, FormatG2Uncompr>(&G2_GENERATOR_MUL_BY_7_SERIALIZED_UNCOMP).extract();
        let point_7g_from_comp =
            deserialize<G2, FormatG2Compr>(&G2_GENERATOR_MUL_BY_7_SERIALIZED_COMP).extract();
        assert!(eq(&point_7g_from_comp, &point_7g_from_uncomp), 1);
    ...
    fun test_gt(fx: signer) {
        ...
        let generator_from_deser =
            deserialize<Gt, FormatGt>(&GT_GENERATOR_SERIALIZED).extract();
        ...
        let identity_from_deser =
            deserialize<Gt, FormatGt>(&FQ12_ONE_SERIALIZED).extract();
        assert!(eq(&identity, &identity_from_deser), 1);
        let element_7g_from_deser =
            deserialize<Gt, FormatGt>(&GT_GENERATOR_MUL_BY_7_SERIALIZED).extract();
        assert!(
            deserialize<Gt, FormatGt>(&x"ffff").is_none(),
            1
        );
    ...
    fun test_fr(fx: signer) {
        ...
        assert!(
            deserialize<Fr, FormatFrLsb>(&x"ffff").is_none(),
            1
        );
        assert!(
            deserialize<Fr, FormatFrMsb>(&x"ffff").is_none(),
            1
        );
    ...
    fun test_fq(fx: signer) {
        ...
        assert!(
            deserialize<Fq, FormatFqLsb>(&x"ffff").is_none(),
            1
        );
        assert!(
            deserialize<Fq, FormatFqMsb>(&x"ffff").is_none(),
            1
        );
