...
module DiemFramework::ParallelExecutionConfig {
    ...
    public fun initialize_parallel_execution(dr_account: &signer) {
    ...
    public fun enable_parallel_execution_with_config(
        dr_account: &signer, read_write_inference_result: vector<u8>
    ) {
    ...
    public fun disable_parallel_execution(dr_account: &signer) {
        DiemTimestamp::assert_operating();
        Roles::assert_diem_root(dr_account);
        DiemConfig::set(
            dr_account,
    ...
    spec initialize_parallel_execution {
        ...
        include DiemConfig::PublishNewConfigEnsures<ParallelExecutionConfig> {
            payload: ParallelExecutionConfig { read_write_analysis_result: option::none() }
        };
    }
    ...
    spec disable_parallel_execution {
        ...
        include DiemConfig::SetAbortsIf<ParallelExecutionConfig> { account: dr_account };
        ensures DiemConfig::spec_is_published<ParallelExecutionConfig>();
        ensures DiemConfig::get<ParallelExecutionConfig>()
            == ParallelExecutionConfig { read_write_analysis_result: option::none() };
        ensures old(DiemConfig::spec_has_config()) == DiemConfig::spec_has_config();
    }
    ...
    spec module {
        invariant [suspendable] forall addr: address where exists<DiemConfig<
            ParallelExecutionConfig>>(addr): addr == @DiemRoot;

        invariant update[suspendable] old(
            DiemConfig::spec_is_published<ParallelExecutionConfig>()
        )
            && DiemConfig::spec_is_published<ParallelExecutionConfig>()
            && old(DiemConfig::get<ParallelExecutionConfig>())
                != DiemConfig::get<ParallelExecutionConfig>() ==>
