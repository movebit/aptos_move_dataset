...
module aptos_token_objects::token {
    ...
    inline fun create_common_with_collection_as_owner(
        ...
        assert!(
            features::is_collection_owner_enabled(),
            error::unavailable(ECOLLECTION_OWNER_NOT_SUPPORTED)
        );
        assert!(
    ...
    inline fun create_common_with_collection_internal(
        ...
        if (name_with_index_suffix.is_some()) {
            ...
            assert!(
                name_prefix.length() + 20 + name_with_index_suffix.borrow().length()
                    <= MAX_TOKEN_NAME_LENGTH,
                error::out_of_range(ETOKEN_NAME_TOO_LONG)
            );
        } else {
            assert!(
                name_prefix.length() <= MAX_TOKEN_NAME_LENGTH,
                error::out_of_range(ETOKEN_NAME_TOO_LONG)
            );
        };
        assert!(
        ...
        let index =
            ...
            ).destroy_with_default(aggregator_v2::create_snapshot<u64>(0));

        // If create_numbered_token called us, add index to the name.
        let name =
            if (name_with_index_suffix.is_some()) {
                aggregator_v2::derive_string_concat(
                    name_prefix, &index, name_with_index_suffix.extract()
                )
            } else {
    ...
    fun test_create_and_transfer_token_as_collection_owner(
        creator: &signer, trader: &signer, aptos_framework: &signer
    ) acquires Token {
        ...
        let extend_ref =
            create_collection_as_collection_owner_helper(creator, collection_name, 1);
    ...
    fun test_create_token_non_collection_owner(
        creator: &signer, trader: &signer, aptos_framework: &signer
    ) {
        features::change_feature_flags_for_testing(
            aptos_framework,
        ...
        let constructor_ref =
            &create_fixed_collection_as_collection_owner(
                creator, string::utf8(b"collection name"), 5
            );
        let collection =
            get_collection_from_ref(&object::generate_extend_ref(constructor_ref));
    ...
    #[test(creator = @0x123, trader = @0x456, aptos_framework = @aptos_framework)]
    #[expected_failure(abort_code = 0x40008, location = aptos_token_objects::token)]
    fun test_create_named_token_non_collection_owner(
        creator: &signer, trader: &signer, aptos_framework: &signer
    ) {
        features::change_feature_flags_for_testing(
            aptos_framework,
        ...
        let collection =
            get_collection_from_ref(&object::generate_extend_ref(constructor_ref));
        create_named_token_as_collection_owner_helper(
            trader, collection, string::utf8(b"token name")
        );
    }
    ...
    #[test(creator = @0x123, trader = @0x456, aptos_framework = @aptos_framework)]
    #[expected_failure(abort_code = 0x40008, location = aptos_token_objects::token)]
    fun test_create_token_after_transferring_collection(
        creator: &signer, trader: &signer, aptos_framework: &signer
    ) {
        features::change_feature_flags_for_testing(
            aptos_framework,
    ...
    #[test(creator = @0x123, trader = @0x456, aptos_framework = @aptos_framework)]
    fun create_token_works_with_new_collection_owner(
        creator: &signer, trader: &signer, aptos_framework: &signer
    ) {
        features::change_feature_flags_for_testing(
            aptos_framework,
    ...
    fun test_upgrade_to_concurrent_and_numbered_tokens(
        ...
        assert!(
            event::emitted_events<collection::Mint>().length() == 2,
            0
        );
        ...
        burn(burn_ref);
        assert!(
            event::emitted_events<collection::Burn>().length() == 1,
            0
        );
    }
    ...
    #[test_only]
    fun create_collection_as_collection_owner_helper(
        creator: &signer, collection_name: String, max_supply: u64
    ): ExtendRef {
        let constructor_ref =
            create_fixed_collection_as_collection_owner(
