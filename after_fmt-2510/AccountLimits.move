...
module DiemFramework::AccountLimits {
    ...
    public fun grant_mutation_capability(dr_account: &signer)
    ...
    spec update_deposit_limits {
        ...
        ensures global<Window<CoinType>>(addr).limit_address
            == old(
                global<Window<CoinType>>(addr).limit_address
            );
        include UpdateDepositLimitsAbortsIf<CoinType>;
        include CanReceiveEnsures<CoinType> {
            receiving: global<Window<CoinType>>(addr)
        };
    }
    ...
    spec update_withdrawal_limits {
        ...
        include CanWithdrawEnsures<CoinType> {
            sending: global<Window<CoinType>>(addr)
        };
    }
    ...
    spec schema PublishWindowEnsures<CoinType> {
        to_limit: signer;
        ensures global<Window<CoinType>>(signer::address_of(to_limit)).window_inflow
            == 0;
        ensures global<Window<CoinType>>(signer::address_of(to_limit)).window_outflow
            == 0;
        ensures global<Window<CoinType>>(signer::address_of(to_limit)).tracked_balance
            == 0;
    }
    ...
    public(friend) fun publish_unrestricted_limits<CoinType>(
    ...
    spec schema PublishUnrestrictedLimitsAbortsIf<CoinType> {
        publish_account: signer;
        aborts_if exists<LimitsDefinition<CoinType>>(signer::address_of(publish_account)) with errors::ALREADY_PUBLISHED;
    }
    ...
    public fun publish_unrestricted_limits_for_testing<CoinType>(
        publish_account: &signer
    ) {
    ...
    public fun update_limits_definition<CoinType>(
        ...
        if (new_max_inflow > 0) {
            limits_def.max_inflow = new_max_inflow
        };
        if (new_max_outflow > 0) {
            limits_def.max_outflow = new_max_outflow
        };
        if (new_max_holding_balance > 0) {
            limits_def.max_holding = new_max_holding_balance
        };
        if (new_time_period > 0) {
            limits_def.time_period = new_time_period
        };
    }
    ...
    spec update_limits_definition {
        ...
        ensures new_max_inflow > 0 ==>
            new_limits_def.max_inflow == new_max_inflow;
        ensures new_max_inflow == 0 ==>
            new_limits_def.max_inflow == old_limits_def.max_inflow;
        ensures new_max_outflow > 0 ==>
            new_limits_def.max_outflow == new_max_outflow;
        ensures new_max_outflow == 0 ==>
            new_limits_def.max_outflow == old_limits_def.max_outflow;
        ensures new_max_holding_balance > 0 ==>
            new_limits_def.max_holding == new_max_holding_balance;
        ensures new_max_holding_balance == 0 ==>
            new_limits_def.max_holding == old_limits_def.max_holding;
        ensures new_time_period > 0 ==>
            new_limits_def.time_period == new_time_period;
        ensures new_time_period == 0 ==>
            new_limits_def.time_period == old_limits_def.time_period;
    }
    ...
    public fun update_window_info<CoinType>(
        ...
        if (aggregate_balance != 0) {
            window.tracked_balance = aggregate_balance
        };
        assert!(
            exists<LimitsDefinition<CoinType>>(new_limit_address),
            errors::not_published(ELIMITS_DEFINITION)
        );
    ...
    spec update_window_info {
        ...
        ensures aggregate_balance != 0 ==>
            new_window.tracked_balance == aggregate_balance;
        ensures aggregate_balance == 0 ==>
            new_window.tracked_balance == old_window.tracked_balance;
    ...
    fun reset_window<CoinType>(
        window: &mut Window<CoinType>, limits_definition: &LimitsDefinition<CoinType>
    ) {
        let current_time = DiemTimestamp::now_microseconds();
        assert!(
            window.window_start <= MAX_U64 - limits_definition.time_period,
            errors::limit_exceeded(EWINDOW)
        );
    ...
    fun can_receive_and_update_window<CoinType>(
        amount: u64, receiving: &mut Window<CoinType>
    ): bool acquires LimitsDefinition {
        assert!(
            exists<LimitsDefinition<CoinType>>(receiving.limit_address),
            errors::not_published(ELIMITS_DEFINITION)
        );
        let limits_definition =
            borrow_global<LimitsDefinition<CoinType>>(receiving.limit_address);
        ...
        assert!(
            receiving.window_inflow <= MAX_U64 - amount,
            errors::limit_exceeded(EWINDOW)
        );
        let inflow_ok = (receiving.window_inflow + amount)
            <= limits_definition.max_inflow;
        // Check that the holding after the deposit is OK
        assert!(
            receiving.tracked_balance <= MAX_U64 - amount,
            errors::limit_exceeded(EWINDOW)
        );
        let holding_ok =
            (receiving.tracked_balance + amount) <= limits_definition.max_holding;
    ...
    spec schema CanReceiveAbortsIf<CoinType> {
        ...
        include !spec_window_unrestricted<CoinType>(receiving) ==>
    ...
    spec schema CanReceiveEnsures<CoinType> {
        ...
        ensures if (result && !spec_window_unrestricted(old(receiving)))
            receiving
                == spec_update_inflow(spec_window_reset(old(receiving)), amount)
        else receiving == spec_window_reset(old(receiving))
    ...
    spec fun spec_update_inflow<CoinType>(receiving: Window<CoinType>, amount: u64): Window<CoinType> {
        update_field(
            ...
            tracked_balance,
            receiving.tracked_balance + amount
        )
    }
    ...
    fun can_withdraw_and_update_window<CoinType>(
        amount: u64, sending: &mut Window<CoinType>
    ): bool acquires LimitsDefinition {
        assert!(
            exists<LimitsDefinition<CoinType>>(sending.limit_address),
            errors::not_published(ELIMITS_DEFINITION)
        );
        let limits_definition =
            borrow_global<LimitsDefinition<CoinType>>(sending.limit_address);
        // If the limits are unrestricted then don't do any more work.
        if (is_unrestricted(limits_definition)) return true;
        ...
        // Check outflow is OK
        assert!(
            sending.window_outflow <= MAX_U64 - amount,
            errors::limit_exceeded(EWINDOW)
        );
        let outflow_ok = sending.window_outflow + amount
            <= limits_definition.max_outflow;
        // Flow is OK, so record it.
        if (outflow_ok) {
            sending.window_outflow = sending.window_outflow + amount;
            sending.tracked_balance =
                if (amount >= sending.tracked_balance) 0
    ...
    spec schema CanWithdrawAbortsIf<CoinType> {
        ...
        include !spec_window_unrestricted(sending) ==>
            CanWithdrawRestrictedAbortsIf<CoinType>;
    }
    ...
    spec schema CanWithdrawEnsures<CoinType> {
        ...
        ensures if (result && !spec_window_unrestricted(old(sending)))
            sending
                == spec_update_outflow(spec_window_reset(old(sending)), amount)
        else sending == spec_window_reset(old(sending)) || sending == old(sending);
    }
    ...
    fun current_time(): u64 {
        if (DiemTimestamp::is_genesis()) 0
        else DiemTimestamp::now_microseconds()
    }
    ...
    spec module {
        ...
        invariant<CoinType> update forall window_addr: address where old(
            exists<Window<CoinType>>(window_addr)
        ): exists<Window<CoinType>>(window_addr);

        /// Invariant that `LimitsDefinition` exists if a `Window` exists.
        invariant<CoinType> forall window_addr: address where exists<Window<CoinType>>(
            window_addr
        ):
            exists<LimitsDefinition<CoinType>>(
                global<Window<CoinType>>(window_addr).limit_address
            );
    }
    ...
    spec module {
        /// Only ParentVASP and ChildVASP can have the account limits [[E1]][ROLE][[E2]][ROLE][[E3]][ROLE][[E4]][ROLE][[E5]][ROLE][[E6]][ROLE][[E7]][ROLE].
        invariant<CoinType> forall addr: address where exists<Window<CoinType>>(addr):
            exists<Roles::RoleId>(addr)
                && (
                    global<Roles::RoleId>(addr).role_id == Roles::PARENT_VASP_ROLE_ID
                        || global<Roles::RoleId>(addr).role_id
