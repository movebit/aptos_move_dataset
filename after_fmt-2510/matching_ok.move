module 0xc0ffee::m {
    ...
    public fun is_inner1(self: &Inner): bool {
        match(self) { Inner1 { x: _ } => true, _ => false }
    }
    ...
    public fun outer_value_with_cond(o: Outer): u64 {
        match(o) {
            ...
            One { i }
            if i.is_inner1() => i.inner_value() % 2,
            One { i } => i.inner_value(),
            Two { i, b } => i.inner_value() + b.x
        }
    }
    ...
    public fun is_some<A>(x: &Option<A>): bool {
        match(x) { None => false, Some { value: _ } => true }
    }
    ...
    fun select_common_fields_different_offset(
        s: CommonFieldsAtDifferentOffset
    ): u64 {
