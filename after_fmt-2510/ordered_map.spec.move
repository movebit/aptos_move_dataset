spec aptos_std::ordered_map {
    ...
    spec remove {
        ...
        ensures [abstract] forall k: K where k != key:
            spec_contains_key(self, k) ==>
                spec_get(self, k) == spec_get(old(self), k);
        ensures [abstract] forall k: K where k != key:
            spec_contains_key(old(self), k) == spec_contains_key(self, k);
    }
    ...
    spec remove_or_none {
        ...
        ensures [abstract] spec_contains_key(old(self), key) ==>
            result == option::spec_some(spec_get(old(self), key));
        ensures [abstract]!spec_contains_key(old(self), key) ==>
            result == option::spec_none();
        ensures [abstract]!spec_contains_key(self, key);
        ensures [abstract] option::spec_is_none(result) ==>
            self == old(self);
        ensures [abstract] option::spec_is_some(result) ==>
            spec_len(old(self)) == spec_len(self) + 1;
        ensures [abstract] forall k: K where k != key:
            spec_contains_key(self, k) ==>
                spec_get(self, k) == spec_get(old(self), k);
        ensures [abstract] forall k: K where k != key:
    ...
    spec keys {
        pragma verify = false;
        pragma opaque;
        ensures [abstract] forall k: K:
            vector::spec_contains(result, k) <==>
                spec_contains_key(self, k);
    }
    ...
    spec upsert {
        ...
        ensures [abstract] spec_contains_key(old(self), key) ==>
            spec_len(old(self)) == spec_len(self);
        ensures [abstract] forall k: K:
            spec_contains_key(old(self), k) && k != key ==>
                spec_get(old(self), k) == spec_get(self, k);
        ensures [abstract] forall k: K:
    ...
    spec borrow_front<K, V>(self: &OrderedMap<K, V>): (&K, &V) {
        ...
        ensures [abstract] forall k: K where k != result_1:
            spec_contains_key(self, k) ==>
    ...
    spec prev_key<K: copy, V>(self: &OrderedMap<K, V>, key: &K): Option<K> {
        ...
        ensures [abstract] result == std::option::spec_none() <==>
            (
                forall k: K { spec_contains_key(self, k) }
                where spec_contains_key(self, k) && k != key:
                    std::cmp::compare(key, k) == std::cmp::Ordering::Less
            );
        ensures [abstract] result.is_some() <==>
            spec_contains_key(self, option::borrow(result))
                && (
                    std::cmp::compare(option::borrow(result), key)
                        == std::cmp::Ordering::Less
                )
                && (
                    forall k: K {
                        spec_contains_key(self, k),
                        std::cmp::compare(option::borrow(result), k),
                        std::cmp::compare(key, k)
                    }
                    where k != option::borrow(result):
                        ((
                            spec_contains_key(self, k)
