...
module 0x42::TestEmits {
    ...
    public fun simple_wrong_msg_incorrect(
    ...
    public fun simple_wrong_handle_incorrect(
        handle: &mut EventHandle<DummyEvent>, _handle2: &mut EventHandle<DummyEvent>
    ) {
        event::emit_event(handle, DummyEvent { msg: 0 });
    }
    ...
    public fun multiple_same_incorrect(
        handle: &mut EventHandle<DummyEvent>
    ) {
        event::emit_event(handle, DummyEvent { msg: 0 });
    }
    ...
    public fun multiple_different_handle(
        handle: &mut EventHandle<DummyEvent>, handle2: &mut EventHandle<DummyEvent>
    ) {
        event::emit_event(handle, DummyEvent { msg: 0 });
        event::emit_event(handle, DummyEvent { msg: 0 });
        event::emit_event(handle, DummyEvent { msg: 1 });
    ...
    public fun conditional_wrong_condition_incorrect(
        x: u64, handle: &mut EventHandle<DummyEvent>
    ) {
        if (x > 7) {
            event::emit_event(handle, DummyEvent { msg: 0 });
        }
    ...
    public fun conditional_missing_condition_incorrect(
        x: u64, handle: &mut EventHandle<DummyEvent>
    ) {
        if (x > 7) {
            event::emit_event(handle, DummyEvent { msg: 0 });
    ...
    public fun conditional_bool(
        b: bool, handle: &mut EventHandle<DummyEvent>
    ) {
        if (b) {
            event::emit_event(handle, DummyEvent { msg: 0 });
        }
    ...
    public fun conditional_multiple_incorrect(
        b: bool, handle: &mut EventHandle<DummyEvent>
    ) {
        event::emit_event(handle, DummyEvent { msg: 0 });
        event::emit_event(handle, DummyEvent { msg: 1 });
    ...
    public fun conditional_multiple_same_incorrect(
        b: bool, handle: &mut EventHandle<DummyEvent>
    ) {
        event::emit_event(handle, DummyEvent { msg: 0 });
        event::emit_event(handle, DummyEvent { msg: 0 });
    ...
    public fun emits_in_schema_condition(
        handle: &mut EventHandle<DummyEvent>, x: u64
    ) {
        if (x > 7) {
    ...
    public fun opaque_completeness_incorrect(
        handle: &mut EventHandle<DummyEvent>
    ) {
        event::emit_event(handle, DummyEvent { msg: 0 });
        callee(handle);
    ...
    public fun opaque_partial_incorrect(
        handle: &mut EventHandle<DummyEvent>
    ) {
        event::emit_event(handle, DummyEvent { msg: 0 });
        callee_partial(handle);
