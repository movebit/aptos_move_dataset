...
module aptos_std::big_ordered_map {
    ...
    public fun contains<K: drop + copy + store, V: store>(
        ...
        else if (&internal_lower_bound.key == key) { true }
        else { false }
    }
    ...
    public fun internal_new_end_iter<K: copy + store, V: store>(
        self: &BigOrderedMap<K, V>
    ): IteratorPtr<K> {
        IteratorPtr::End
    }
    ...
    public fun iter_remove<K: drop + copy + store, V: store>(
        ...
        let Child::Leaf { value } = map.remove_at(path_to_leaf, &key);
        value
    }
    ...
    fun validate_size_and_init_max_degrees<K: store, V: store>(
        ...
        if (self.leaf_max_degree == 0) {
            let default_max_degree =
                min(
                    MAX_DEGREE,
                    MAX_NODE_BYTES / DEFAULT_MAX_KEY_OR_VALUE_SIZE / 2
                );
            self.leaf_max_degree =
                max(
        ...
        if (expected_lower_bound_key.is_some()) {
            ...
            assert!(
                cmp::compare(
                    ...
                    node.children.internal_new_begin_iter().iter_borrow_key(
                        &node.children
                    )
                ).is_lt(),
                error::invalid_state(EINTERNAL_INVARIANT_BROKEN)
    ...
    fun test_modify_and_get() {
        ...
        assert!(
            map.get_and_map(&2, |v| *v + 5) == option::some(17)
        );
        ...
        assert!(
            option::some(7)
                == map.modify_if_present_and_return(&4, |v| {
                    *v += 10;
                    7
                })
        );
        ...
        assert!(
            option::none()
                == map.modify_if_present_and_return(&5, |v| {
                    *v += 10;
                    7
                })
        );
    ...
    #[test]
    #[expected_failure(abort_code = 0x10002, location = aptos_std::ordered_map)]
    /// EKEY_NOT_FOUND
    ...
    /// EBORROW_MUT_REQUIRES_CONSTANT_VALUE_SIZE
