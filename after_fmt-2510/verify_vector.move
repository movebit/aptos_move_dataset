...
module 0x42::VerifyVector {
    ...
    fun verify_model_push_back<Element>(
    ...
    fun verify_append<Element>(
        v: &mut vector<Element>, other: &mut vector<Element>
    ) {
        ...
        while ({
            spec {
                ...
                invariant forall k in 0..len(other):
                    other[k] == old(other)[len(old(other)) - 1 - k];
                invariant forall k in len(old(v))..len(v):
                    v[k] == old(other)[k - len(old(v))];
            };
    ...
    fun verify_append_with_unroll<Element>(
        v: &mut vector<Element>, other: vector<Element>
    ) {
        let o = &mut other;
        vector::reverse(o);
    ...
    fun verify_model_append<Element>(
    ...
    fun verify_index_of_with_unroll<Element>(
        v: &vector<Element>, e: &Element
    ): (bool, u64) {
        let i = 0;
        let len = vector::length(v);
        while (i < len) {
    ...
    fun verify_contains_with_unroll<Element>(
        v: &vector<Element>, e: &Element
    ): bool {
        let i = 0;
        let len = vector::length(v);
    ...
    fun verify_remove_with_unroll<Element>(
        v: &mut vector<Element>, j: u64
    ): Element {
        let vlen = vector::length(v);
        let i = j;
        // i out of bounds; abort
    ...
    fun vector_operator_in_function<Element: copy + drop>(
        e1: Element, e2: Element, e3: Element
    ): vector<Element> {
        vector[e1, e2, e3]
    }

    fun spec_with_vector_operator<Element: copy + drop>(
        e1: Element, e2: Element, e3: Element
    ...
    spec spec_with_vector_operator {
        ...
        ensures result_1
            == concat(
                vector_operator_in_function(e1, e2, e3),
                vector[e1, e2]
            );
    }
}
