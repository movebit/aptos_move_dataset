...
module post_mint_reveal_nft::whitelist {
    ...
    public fun get_num_of_stages(module_address: address): u64 acquires WhitelistMintConfig {
        vector::length(
            &borrow_global<WhitelistMintConfig>(module_address).whitelist_configs
        )
    }
    ...
    public fun is_user_currently_eligible_for_whitelisted_minting(
        module_address: address, minter_address: address
    ): (u64, u64, bool) acquires WhitelistMintConfig {
        ...
        while (i < vector::length(&whitelist_mint_config.whitelist_configs)) {
            let whitelist_stage = vector::borrow(
            ...
            if (whitelist_stage.whitelist_minting_start_time <= now
                ...
                return (
                    whitelist_stage.whitelist_mint_price,
                    i,
                    user_is_eligible_for_current_whitelisted_minting
                )
            };
            i = i + 1;
        };
    ...
    public(friend) fun deduct_user_minting_amount(
        ...
        let remaining_minting_amount =
            bucket_table::borrow_mut(
                &mut whitelist_stage.whitelisted_address, minter_address
            );
        assert!(
    ...
    public fun add_or_update_whitelist_stage(
        admin: &signer,
