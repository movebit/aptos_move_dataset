...
module aptos_std::smart_table {
    ...
    inline fun unzip_entries<K: copy, V: copy>(
        entries: &vector<Entry<K, V>>
    ): (vector<K>, vector<V>) {
        let keys = vector[];
        let values = vector[];
        entries.for_each_ref(
            |e| {
    ...
    public fun borrow<K: drop, V>(self: &SmartTable<K, V>, key: K): &V {
        let index = bucket_index(
            self.level, self.num_buckets, sip_hash_from_value(&key)
        );
    ...
    public fun contains<K: drop, V>(self: &SmartTable<K, V>, key: K): bool {
        ...
        let index = bucket_index(self.level, self.num_buckets, hash);
        let bucket = self.buckets.borrow(index);
        bucket.any(|e| { e.hash == hash && &e.key == &key })
    }
    ...
    public inline fun for_each_mut<K, V>(
        self: &mut SmartTable<K, V>, f: |&K, &mut V|
    ) {
        for (i in 0..self.num_buckets()) {
            self.borrow_buckets_mut().borrow_mut(i).for_each_mut(
                |elem| {
    ...
    public inline fun any<K, V>(self: &SmartTable<K, V>, p: |&K, &V| bool): bool {
        let found = false;
        for (i in 0..self.num_buckets()) {
            found = self.borrow_buckets().borrow(i).any(
                |elem| {
                    let (key, value) = elem.borrow_kv();
                    p(key, value)
                }
    ...
    fun test_keys() {
        ...
        loop {
            (returned_keys, starting_bucket_index_r, starting_vector_index_r) = table.keys_paginated(
                starting_bucket_index, starting_vector_index, 15
            );
