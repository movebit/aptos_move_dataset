...
module DiemFramework::DiemSystem {
    ...
    spec DiemSystem {
        ...
        invariant forall i in 0..len(validators), j in 0..len(validators):
    ...
    public fun add_validator(
        ...
        vector::push_back(
            &mut diem_system_config.validators,
            ValidatorInfo {
                addr: validator_addr,
    ...
    public fun remove_validator(
        ...
        let to_remove_index_vec =
            get_validator_index_(&diem_system_config.validators, validator_addr);
        assert!(
            option::is_some(&to_remove_index_vec),
    ...
    spec schema RemoveValidatorEnsures {
        ...
        let vs = spec_get_validators();
        let post post_vs = spec_get_validators();
        ensures forall vi in post_vs where vi.addr != validator_addr:
            exists ovi in vs: vi == ovi;
    ...
    spec update_config_and_reconfigure {
        ...
        let is_validator_info_updated = ValidatorConfig::is_valid(validator_addr)
            && (
                exists v_info in spec_get_validators():
                    v_info.addr == validator_addr
                        && v_info.config
                            != ValidatorConfig::spec_get_config(validator_addr)
            );
        include is_validator_info_updated ==>
            DiemConfig::ReconfigureAbortsIf;
        let validator_index = spec_index_of_validator(
            spec_get_validators(), validator_addr
        );
        let last_config_time = spec_get_validators()[validator_index].last_config_update_time;
        aborts_if is_validator_info_updated
            && last_config_time > MAX_U64 - FIVE_MINUTES with errors::LIMIT_EXCEEDED;
        aborts_if is_validator_info_updated
            && DiemTimestamp::spec_now_microseconds()
                <= last_config_time + FIVE_MINUTES with errors::LIMIT_EXCEEDED;
    ...
    spec schema UpdateConfigAndReconfigureAbortsIf {
        ...
        aborts_if ValidatorConfig::get_operator(validator_addr)
            != validator_operator_addr with errors::INVALID_ARGUMENT;
        aborts_if !spec_is_validator(validator_addr) with errors::INVALID_ARGUMENT;
    }
    ...
    spec schema UpdateConfigAndReconfigureEnsures {
        ...
        ensures forall i in 0..len(vs):
            post_vs[i].config == vs[i].config
                || (
                    vs[i].addr == validator_addr
                        && post_vs[i].config
                            == ValidatorConfig::get_config(validator_addr)
                );
        /// DIP-6 property
        ensures Roles::spec_has_validator_role_addr(validator_addr);
    }
    ...
    spec get_validator_index_ {
        ...
        ensures (forall i in 0..size: validators[i].addr != addr) ==>
    ...
    fun update_ith_validator_info_(
        validators: &mut vector<ValidatorInfo>, i: u64
    ): bool {
        let size = vector::length(validators);
    ...
    fun is_validator_(
        addr: address, validators_vec_ref: &vector<ValidatorInfo>
    ): bool {
    ...
    spec module {
        invariant forall addr: address where exists<DiemConfig<DiemSystem>>(addr):
            addr == @DiemRoot;
        ...
        invariant update[suspendable](
            old(DiemConfig::spec_is_published<DiemSystem>())
                && DiemConfig::spec_is_published<DiemSystem>()
                && old(
                    len(DiemConfig::get<DiemSystem>().validators)
                ) != len(DiemConfig::get<DiemSystem>().validators)
        ) ==>
                        ...
                        (
                            exists a: address:
                                signer::is_txn_signer_addr(a)
                                    && ValidatorConfig::get_operator(addr) == a
                        )
            );
    }
    ...
    spec module {
        ...
        /// Genesis.
        apply ValidatorSetConfigRemainsSame to *,*<T> except add_validator, remove_validator, update_config_and_reconfigure, initialize_validator_set, set_diem_system_config;
    }
