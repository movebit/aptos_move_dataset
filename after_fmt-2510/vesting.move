...
module aptos_framework::vesting {
    ...
    public fun grant_permission(
        ...
        permissioned_signer::authorize_unlimited(
            master, permissioned_signer, VestPermission {}
        )
    }
    ...
    public fun create_vesting_contract(
        ...
        vector::for_each_ref(
            ...
            |shareholder| {
                ...
                pool_u64::buy_in(&mut grant_pool, shareholder, buy_in_amount);
    ...
    public entry fun reset_lockup(
        ...
        verify_admin(admin, vesting_contract);
        let contract_signer = &get_vesting_account_signer_internal(vesting_contract);
        staking_contract::reset_lockup(
            contract_signer, vesting_contract.staking.operator
        );
    ...
    fun withdraw_stake(
        ...
        staking_contract::distribute(
            contract_address, vesting_contract.staking.operator
        );
    ...
    public fun setup(
        ...
        std::features::change_feature_flags_for_testing(
            aptos_framework,
            vector[],
            vector[std::features::get_periodical_reward_rate_decrease_feature()]
        );
    }
    ...
    public entry fun test_end_to_end(
        ...
        stake::end_epoch();
        assert!(
            stake::get_validator_state(stake_pool_address) == VALIDATOR_STATUS_ACTIVE,
            2
        );
        unlock_rewards(contract_address);
        vest(contract_address);
    ...
    public entry fun test_vest_twice_should_not_double_count(
        aptos_framework: &signer, admin: &signer, shareholder: &signer
    ) acquires AdminStore, VestingContract {
        let admin_address = signer::address_of(admin);
    ...
    public entry fun test_unlock_rewards_should_pay_commission_first(
        ...
        assert!(
            coin::balance<AptosCoin>(shareholder_address) == staker_rewards + 1,
            0
        );
        assert!(
    ...
    public entry fun test_request_commission_should_not_lock_rewards_for_shareholders(
        ...
        staking_contract::request_commission(
            operator, contract_address, operator_address
        );
    ...
    public entry fun test_update_operator_with_same_commission(
        aptos_framework: &signer, admin: &signer, operator: &signer
    ) acquires AdminStore, VestingContract {
        let admin_address = signer::address_of(admin);
        let operator_address = signer::address_of(operator);
        setup(
            aptos_framework,
            &vector[admin_address, @11, operator_address]
        );
        let contract_address =
            setup_vesting_contract(
                admin,
    ...
    public entry fun test_set_beneficiary_with_unregistered_account(
        ...
        std::features::change_feature_flags_for_testing(
            aptos_framework, vector[], vector[fa_feature]
        );
        let admin_address = signer::address_of(admin);
        setup(aptos_framework, &vector[admin_address]);
        let contract_address =
    ...
    public entry fun test_reset_beneficiary_with_resetter_role(
        aptos_framework: &signer, admin: &signer, resetter: &signer
    ) acquires AdminStore, VestingAccountManagement, VestingContract {
        let admin_address = signer::address_of(admin);
        setup(
            aptos_framework,
    ...
    #[test(
        aptos_framework = @0x1, admin = @0x123, resetter = @0x234, random = @0x345
    )]
