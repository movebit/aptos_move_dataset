...
module aptos_framework::ten_x_token_tests {
    ...
    #[test(creator = @0xcafe)]
    fun test_ten_x_pfs(creator: &signer) {
        let (creator_ref, token_object) = fungible_asset::create_test_token(creator);
        let (mint, _, _) =
        ...
        assert!(
            primary_fungible_store::is_balance_at_least(creator_address, metadata, 1000),
            4
        );
    }
    ...
    fun ten_x_balance_abort(creator: &signer) {
        let (creator_ref, token_object) = fungible_asset::create_test_token(creator);
    ...
    #[test(creator = @0xcafe)]
    #[expected_failure(abort_code = 0x1001C, location = aptos_framework::fungible_asset)]
    fun ten_x_supply_abort(creator: &signer) {
        let (creator_ref, token_object) = fungible_asset::create_test_token(creator);
        fungible_asset::init_test_metadata(&creator_ref);
        let metadata = object::convert<TestToken, Metadata>(token_object);
