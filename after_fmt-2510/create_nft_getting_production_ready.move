...
module mint_nft::create_nft_getting_production_ready {
    ...
    fun init_module(resource_signer: &signer) {
        ...
        let pk_bytes =
            x"f66bf0ce5ceb582b93d6780820c2025b9967aedaa259bdbb9f3d0297eced0e18";
        let public_key =
            std::option::extract(
                &mut ed25519::new_validated_public_key_from_bytes(pk_bytes)
            );
        move_to(
            resource_signer,
            ModuleData {
    ...
    public entry fun mint_event_ticket(
        receiver: &signer, mint_proof_signature: vector<u8>
    ) acquires ModuleData {
    ...
    public entry fun set_minting_enabled(
        caller: &signer, minting_enabled: bool
    ) acquires ModuleData {
    ...
    public entry fun set_timestamp(
        caller: &signer, expiration_timestamp: u64
    ) acquires ModuleData {
        let caller_address = signer::address_of(caller);
        assert!(caller_address == @admin_addr, error::permission_denied(ENOT_AUTHORIZED));
        let module_data = borrow_global_mut<ModuleData>(@mint_nft);
    ...
    public entry fun set_public_key(caller: &signer, pk_bytes: vector<u8>) acquires ModuleData {
        ...
        assert!(caller_address == @admin_addr, error::permission_denied(ENOT_AUTHORIZED));
        let module_data = borrow_global_mut<ModuleData>(@mint_nft);
        module_data.public_key = std::option::extract(
            &mut ed25519::new_validated_public_key_from_bytes(pk_bytes)
        );
    }
    ...
    fun verify_proof_of_knowledge(
        ...
        assert!(
            ed25519::signature_verify_strict_t(
                &signature, &unvalidated_public_key, proof_challenge
            ),
    ...
    public fun set_up_test(
        ...
        let pk_bytes =
            ed25519::validated_public_key_to_bytes(collection_token_minter_public_key);
    ...
    public entry fun test_happy_path(
        ...
        let proof_challenge_2 = MintProofChallenge {
            receiver_account_sequence_number: account::get_sequence_number(
                receiver_addr_2
            ),
