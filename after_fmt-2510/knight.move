...
module knight::knight {
    ...
    public fun knight_token_address(knight_token_name: String): address {
        token::create_token_address(
            &@knight,
            &string::utf8(KNIGHT_COLLECTION_NAME),
            &knight_token_name
        )
    }
    ...
    public entry fun mint_knight(
        ...
        let knight_token = KnightToken { mutator_ref, property_mutator_ref, base_uri };
        move_to(&object_signer, knight_token);
    }

    public entry fun feed_corn(
        from: &signer, to: Object<KnightToken>, amount: u64
    ...
    public fun test_knight(creator: &signer, user1: &signer) acquires HealthPoint, KnightToken {
        ...
        assert!(
            food::food_balance(user1_addr, corn_token) == old_corn_balance - 3, 0
        );
        ...
        // Asserts that the corn balance decreases by 3.
        assert!(
            food::food_balance(user1_addr, meat_token) == old_meat_balance - 2, 0
        );
