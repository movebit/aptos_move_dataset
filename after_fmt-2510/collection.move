...
module aptos_token_objects::collection {
    ...
    public fun create_unlimited_collection_as_owner(
        ...
        let constructor_ref =
            create_unlimited_collection(creator, description, name, royalty, uri);
        enable_ungated_transfer(&constructor_ref);
        constructor_ref
    }
    ...
    spec increment_supply {
        ...
        ensures aggregator_v2::spec_get_max_value(supply.current_supply)
    ...
    public fun set_max_supply(
        mutator_ref: &MutatorRef, max_supply: u64
    ...
    fun test_create_mint_burn_for_unlimited(
        ...
        let cid =
            aggregator_v2::read_snapshot(
                &increment_supply(&collection, creator_address).destroy_some()
            );
    ...
    entry fun test_create_and_transfer_as_owner(
    ...
    #[test(creator = @0x123)]
    entry fun test_set_max_supply_concurrent(
        creator: &signer
    ) acquires ConcurrentSupply, FixedSupply {
        let collection_name = string::utf8(b"collection name");
        let max_supply = 100;
    ...
    #[test_only]
    fun create_unlimited_collection_as_owner_helper(
        creator: &signer, name: String
    ): ConstructorRef {
    ...
    public fun get_mint_event_data(event: &Mint): (address, u64, address) {
        (event.collection, aggregator_v2::read_snapshot(&event.index), event.token)
    }

    #[test_only]
    /// Get all data from SetMaxSupply event
    public fun get_set_max_supply_event_data(event: &SetMaxSupply)
