...
module aptos_experimental::bulk_order_book {
    ...
    use aptos_experimental::bulk_order_book_types::{
        ...
        get_sequence_number_from_order_request,
        get_sequence_number_from_bulk_order
    };
    use aptos_experimental::order_book_types::{
        OrderMatch,
        OrderMatchDetails,
    ...
    public fun get_single_match_for_taker<M: store + copy + drop>(
        ...
        let order_match = new_bulk_order_match<M>(&mut order, !is_bid, matched_size);
        let (next_price, next_size) =
            order.match_order_and_get_next(!is_bid, matched_size);
    ...
    public fun get_remaining_size<M: store + copy + drop>(
        ...
        let result_option =
            self.orders.get_and_map(
                &account, |order| order.get_total_remaining_size(is_bid)
            );
    ...
    public fun get_prices<M: store + copy + drop>(
        ...
        let result_option =
            self.orders.get_and_map(&account, |order| order.get_all_prices(is_bid));
        assert!(result_option.is_some(), EORDER_NOT_FOUND);
        result_option.destroy_some()
    }
    ...
    public fun get_sizes<M: store + copy + drop>(
        ...
        let result_option =
            self.orders.get_and_map(&account, |order| order.get_all_sizes(is_bid));
        assert!(result_option.is_some(), EORDER_NOT_FOUND);
        result_option.destroy_some()
    }
        ...
        let (order_id, previous_seq_num) =
            if (order_option.is_some()) {
                ...
                let existing_sequence_number =
                    get_sequence_number_from_bulk_order(&old_order);
                ...
                let order_id =
                    new_order_id_type(ascending_id_generator.next_ascending_id());
