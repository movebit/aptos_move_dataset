...
module 0x42::Test {
    ...
    fun new_event_handle_impl<T: copy + drop + store>(
        counter: &mut EventHandleGenerator, sender: address
    ): EventHandle<T> {
    ...
    public fun create(
        sender: &signer, fresh_address: address, auth_key_prefix: vector<u8>
    ): vector<u8> {
        ...
        move_to<T>(
            sender,
            T {
                received_events: new_event_handle_impl<Event1>(
