...
module 0x42::mod3 {
    ...
    public fun test_items(owner: signer, use_1: bool): bool {
        let f1: |address| bool has store + copy + drop =
            |addr| 0x42::mod2::item_exists<MyStruct1>(addr);
        let f2: |address| bool has store + copy + drop =
            |addr| 0x42::mod2::item_exists<MyStruct2>(addr);
        ...
        assert!(
            0x42::mod2::item_exists<|address| bool has store + copy + drop>(addr)
        );
...
module 0x42::mod4 {
    ...
    fun initialize(acc: &signer) {
        move_to<Wrapper<|&u64, &mut u8| &u64 has copy + store + drop>>(
            acc, Wrapper { fv: 0x42::mod4::test }
        );
    }
...
module 0x42::mod5 {
    ...
    fun initialize(acc: &signer) {
        move_to<VecWrapper<|&u64, &mut u8| &u64 has copy + store + drop>>(
            acc, VecWrapper { fvs: vector[0x42::mod5::test] }
        );
    }

    fun check_exists(_acc: &signer) {
        let exists = exists<VecWrapper<|&u64, &mut u8| &u64 has copy + store + drop>>(
            @0x42
        );
        assert!(exists, 404);
    }
...
module 0x42::mod6 {
    ...
    fun initialize(acc: &signer) {
        let fvs = vector[0x42::mod6::test1, 0x42::mod6::test2, 0x42::mod6::test3];
        move_to<VecWrapper<|&mut u8| has copy + store + drop>>(acc, VecWrapper { fvs });
    }

    fun compute(_acc: &signer): u8 {
        let do_not_exist =
            !exists<VecWrapper<|&mut u8| has store + drop>>(@0x42)
                && !exists<VecWrapper<|&mut u8| has store>>(@0x42);
        assert!(do_not_exist, 404);

        let wrapper = &borrow_global<VecWrapper<|&mut u8| has copy + store + drop>>(@0x42)
            .fvs;
        let x = 0;
