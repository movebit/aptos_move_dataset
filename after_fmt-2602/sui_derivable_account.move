...
module aptos_framework::sui_derivable_account {
    ...
    fun derive_account_address_from_public_key(
        signing_scheme: u8, public_key_bytes: vector<u8>
    ): vector<u8> {
        ...
        while (i < sui_account_address.length()) {
            ...
            let hex_chars = vector[
                if ((byte >> 4) < 10) ((byte >> 4) + 0x30) else ((byte >> 4) - 10
                    + 0x61),
                if ((byte & 0xf) < 10) ((byte & 0xf) + 0x30) else ((byte & 0xf) - 10
                    + 0x61)
            ];
    ...
    public fun authenticate_auth_data(
        ...
        let (signing_scheme, abstract_signature_signature, abstract_signature_public_key) =
            split_signature_bytes(&abstract_signature.signature);

        // Check siging scheme is Ed25519 as we currently only support this scheme
        assert!(
            get_signing_scheme(signing_scheme) == SuiSigningScheme::ED25519,
            EINVALID_SIGNING_SCHEME_TYPE
        );

        // Derive the account address from the public key
        let sui_account_address =
            derive_account_address_from_public_key(
                signing_scheme, abstract_signature_public_key
            );
        ...
        assert!(
            &sui_account_address == &abstract_public_key.sui_account_address,
            EACCOUNT_ADDRESS_MISMATCH
        );

        let public_key =
            new_validated_public_key_from_bytes(abstract_signature_public_key);
        ...
        let intent = Intent { scope: PersonalMessage, version: V0, app_id: Sui };
        let msg = IntentMessage { intent, value: raw_message };
    ...
    public fun authenticate(
        ...
        daa_authenticate(
            account,
            aa_auth_data,
            |auth_data, entry_name| authenticate_auth_data(auth_data, entry_name)
        )
    }
    ...
    fun create_abstract_public_key(
        sui_account_address: vector<u8>, domain: vector<u8>
    ): vector<u8> {
        let abstract_public_key = SuiAbstractPublicKey { sui_account_address, domain };
    ...
    fun test_derive_account_address_from_public_key() {
        let sui_public_key = vector[
            25, 200, 235, 92, 139, 72, 175, 189, 40, 0, 65, 76, 215, 148, 94, 194, 78, 134,
            60, 189, 212, 116, 40, 134, 179, 104, 31, 249, 222, 84, 104, 202
        ];
        let account_address = derive_account_address_from_public_key(
            0x00, sui_public_key
        );
        assert!(
            account_address
                == b"0x8d6ce7a3c13617b29aaf7ec58bee5a611606a89c62c5efbea32e06d8d167bd49"
        );
    }
    ...
    fun test_deserialize_abstract_signature() {
        let signature_bytes = vector[
            0, 151, 47, 171, 144, 115, 16, 129, 17, 202, 212, 180, 155, 213, 223, 249, 203,
            195, 0, 84, 142, 121, 167, 29, 113, 159, 33, 177, 108, 137, 113, 160, 118, 41,
            246, 199, 202, 79, 151, 27, 86, 235, 219, 123, 168, 152, 38, 124, 147, 146,
            118, 101, 37, 187, 223, 206, 120, 101, 148, 33, 141, 80, 60, 155, 13, 25, 200,
            235, 92, 139, 72, 175, 189, 40, 0, 65, 76, 215, 148, 94, 194, 78, 134, 60, 189,
            212, 116, 40, 134, 179, 104, 31, 249, 222, 84, 104, 202
        ];
    ...
    fun test_split_signature_bytes() {
        ...
        assert!(
            signature
                == vector[
                    151, 47, 171, 144, 115, 16, 129, 17, 202, 212, 180, 155, 213, 223, 249,
                    203, 195, 0, 84, 142, 121, 167, 29, 113, 159, 33, 177, 108, 137, 113,
                    160, 118, 41, 246, 199, 202, 79, 151, 27, 86, 235, 219, 123, 168, 152,
                    38, 124, 147, 146, 118, 101, 37, 187, 223, 206, 120, 101, 148, 33, 141,
                    80, 60, 155, 13
                ]
        );
        assert!(
            public_key
                == vector[
                    25, 200, 235, 92, 139, 72, 175, 189, 40, 0, 65, 76, 215, 148, 94, 194,
                    78, 134, 60, 189, 212, 116, 40, 134, 179, 104, 31, 249, 222, 84, 104,
                    202
                ]
        );
    }
    ...
    fun test_authenticate_auth_data(framework: &signer) {
        ...
        let abstract_public_key = create_abstract_public_key(
            sui_account_address, domain
        );

        let signature_bytes = vector[
            0, 140, 147, 99, 142, 194, 60, 242, 45, 231, 203, 175, 182, 126, 202, 88, 32,
            157, 255, 80, 200, 28, 135, 142, 3, 1, 58, 192, 53, 166, 235, 171, 168, 32,
            163, 200, 137, 125, 161, 149, 149, 159, 254, 116, 51, 159, 23, 11, 196, 173,
            127, 7, 214, 231, 235, 171, 224, 221, 229, 219, 27, 31, 80, 173, 12, 25, 200,
            235, 92, 139, 72, 175, 189, 40, 0, 65, 76, 215, 148, 94, 194, 78, 134, 60, 189,
            212, 116, 40, 134, 179, 104, 31, 249, 222, 84, 104, 202
        ];
        let abstract_signature = create_raw_signature(signature_bytes);
    ...
    fun test_authenticate_auth_data_invalid_signature(
        ...
        let signature_bytes = vector[
            0, 141, 147, 99, 142, 194, 60, 242, 45, 231, 203, 175, 182, 126, 202, 88, 32,
            157, 255, 80, 200, 28, 135, 142, 3, 1, 58, 192, 53, 166, 235, 171, 168, 32,
            163, 200, 137, 125, 161, 149, 149, 159, 254, 116, 51, 159, 23, 11, 196, 173,
            127, 7, 214, 231, 235, 171, 224, 221, 229, 219, 27, 31, 80, 173, 12, 25, 200,
            235, 92, 139, 72, 175, 189, 40, 0, 65, 76, 215, 148, 94, 194, 78, 134, 60, 189,
    ...
    fun test_authenticate_auth_data_invalid_signing_scheme_type(
        framework: &signer
    ) {
        chain_id::initialize_for_test(framework, 2);
        ...
        let signature_bytes = vector[
            1, 140, 147, 99, 142, 194, 60, 242, 45, 231, 203, 175, 182, 126, 202, 88, 32,
            157, 255, 80, 200, 28, 135, 142, 3, 1, 58, 192, 53, 166, 235, 171, 168, 32,
            163, 200, 137, 125, 161, 149, 149, 159, 254, 116, 51, 159, 23, 11, 196, 173,
            127, 7, 214, 231, 235, 171, 224, 221, 229, 219, 27, 31, 80, 173, 12, 25, 200,
            235, 92, 139, 72, 175, 189, 40, 0, 65, 76, 215, 148, 94, 194, 78, 134, 60, 189,
    ...
    fun test_authenticate_auth_data_invalid_account_address_mismatch(
        framework: &signer
    ) {
        chain_id::initialize_for_test(framework, 2);
        ...
        let signature_bytes = vector[
            ...
            127, 7, 214, 231, 235, 171, 224, 221, 229, 219, 27, 31, 80, 173, 12, 25, 200,
            235, 92, 139, 72, 175, 189, 40, 0, 65, 76, 215, 148, 94, 194, 78, 134, 60, 189,
            212, 116, 40, 134, 179, 104, 31, 249, 222, 84, 104, 201
        ];
        let abstract_signature = create_raw_signature(signature_bytes);
