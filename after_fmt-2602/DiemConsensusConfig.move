...
module DiemFramework::DiemConsensusConfig {
    ...
    public fun initialize(dr_account: &signer) {
        ...
        DiemConfig::publish_new_config(
            dr_account, DiemConsensusConfig { config: vector::empty() }
        );
    }
    ...
    public fun set(dr_account: &signer, config: vector<u8>) {
        ...
        DiemConfig::set(dr_account, DiemConsensusConfig { config });
    }
    ...
    spec set {
        ...
        include DiemConfig::SetEnsures<DiemConsensusConfig> {
            payload: DiemConsensusConfig { config }
        };
    }
    ...
    spec module {
        invariant [suspendable] forall addr: address where exists<DiemConfig<
            DiemConsensusConfig>>(addr): addr == @DiemRoot;

        invariant update[suspendable] old(
            DiemConfig::spec_is_published<DiemConsensusConfig>()
        )
            && DiemConfig::spec_is_published<DiemConsensusConfig>()
            && old(DiemConfig::get<DiemConsensusConfig>())
                != DiemConfig::get<DiemConsensusConfig>() ==>
