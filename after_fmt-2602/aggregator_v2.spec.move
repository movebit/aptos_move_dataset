spec aptos_framework::aggregator_v2 {
    ...
    spec create_aggregator<IntElement: copy + drop>(max_value: IntElement)
    ...
    spec is_at_least_impl<IntElement>(
    ...
    spec snapshot<IntElement>(self: &Aggregator<IntElement>)
    ...
    spec create_snapshot<IntElement: copy + drop>(value: IntElement)
        : AggregatorSnapshot<IntElement> {
        pragma opaque;
        include AbortsIfIntElement<IntElement>;
    ...
    spec derive_string_concat<IntElement>(
        before: String, snapshot: &AggregatorSnapshot<IntElement>, after: String
    ): DerivedStringSnapshot {
        pragma opaque;
        include AbortsIfIntElement<IntElement>;
        ensures [abstract] result.value.bytes
            == concat(
                before.bytes,
                concat(spec_get_string_value(snapshot).bytes, after.bytes)
            );
    ...
    spec schema AbortsIfIntElement<IntElement> {
        ...
        aborts_if [abstract] type_info::type_name<IntElement>().bytes != b"u64"
    ...
    spec fun spec_read_snapshot<IntElement>(
