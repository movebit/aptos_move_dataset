...
module DiemFramework::CRSN {
    ...
    public(friend) fun record(account: &signer, sequence_nonce: u64): bool acquires CRSN {
    ...
    public(friend) fun check(account: &signer, sequence_nonce: u64): bool acquires CRSN {
        let addr = signer::address_of(account);
    ...
    public fun force_expire(account: &signer, shift_amount: u64) acquires CRSN {
    ...
    #[test_only]
    public fun test_record(account: &signer, sequence_nonce: u64): bool acquires CRSN {
        record(account, sequence_nonce)
    }

    #[test_only]
    public fun test_check(account: &signer, sequence_nonce: u64): bool acquires CRSN {
        check(account, sequence_nonce)
    }

    #[test_only]
    public fun test_force_expire(account: &signer, shift_amount: u64) acquires CRSN {
        force_expire(account, shift_amount)
    }

    #[test_only]
    public fun slots(account: address): BitVector acquires CRSN {
        *&borrow_global<CRSN>(account).slots
    }

    #[test_only]
    public fun min_nonce(account: address): u64 acquires CRSN {
        *&borrow_global<CRSN>(account).min_nonce
    }

    #[test_only]
    public fun size(account: address): u64 acquires CRSN {
