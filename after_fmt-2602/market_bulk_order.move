...
module aptos_experimental::market_bulk_order {
    ...
    use aptos_experimental::market_types::{MarketClearinghouseCallbacks, Market};
    ...
    public fun place_bulk_order<M: store + copy + drop, R: store + copy + drop>(
        ...
        let (request_option, request_rejection_reason_option) =
            destroy_bulk_order_request_response(request_response);
        ...
        if (is_bulk_order_success_response(&response)) {
            ...
            ) = destroy_bulk_order_place_success_response(response);
            let (
                order_id,
                _,
                _,
                ...
                ask_prices,
                _
            ) = bulk_order.destroy_bulk_order(); // We don't need to keep the bulk order struct after placement
            ...
            let previous_seq_num =
                if (previous_seq_num_option.is_some()) {
    ...
    public(package) fun cancel_bulk_order_internal<M: store + copy + drop, R: store + copy
        + drop>(
        market: &mut Market<M>,
        user: address,
        callbacks: &MarketClearinghouseCallbacks<M, R>
