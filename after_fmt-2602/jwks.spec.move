spec aptos_framework::jwks {
    ...
    spec remove_issuer(jwks: &mut AllProvidersJWKs, issuer: vector<u8>)
        ...
        ensures option::is_none(result) <==>
            (
                forall jwk: ProviderJWKs where vector::spec_contains(
                    old(jwks).entries, jwk
                ): jwk.issuer != issuer
            );
        ensures option::is_none(result) ==> old(jwks) == jwks;
        ensures option::is_some(result) ==>
            vector::spec_contains(old(jwks).entries, option::borrow(result));
    }
}
