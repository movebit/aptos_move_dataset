module aptos_framework::permissioned_delegation {
    ...
    inline fun check_txn_rate(
        bundle: &mut AccountDelegation, check_rate_limit: bool
    ) {
        let token_bucket = &mut bundle.rate_limiter;
        if (check_rate_limit && token_bucket.is_some()) {
            assert!(
                rate_limiter::request(token_bucket.borrow_mut(), 1),
                std::error::permission_denied(ERATE_LIMITED)
            );
        };
    }
    ...
    public fun add_permissioned_handle(
        ...
        let handle =
            permissioned_signer::create_storable_permissioned_handle(
                master, expiration_time
            );
    ...
    public fun remove_permissioned_handle(
        master: &signer, key: DelegationKey
    ...
    public fun handle_address_by_key(
        master: address, key: DelegationKey
    ): address acquires RegisteredDelegations {
    ...
    public fun authenticate(
        account: signer, abstraction_auth_data: AbstractionAuthData
    ): signer acquires RegisteredDelegations {
        ...
        let handle =
            get_storable_permissioned_handle(
                addr, DelegationKey::Ed25519PublicKey(public_key), true
            );
        permissioned_signer::signer_from_storable_permissioned_handle(handle)
    }

    inline fun get_storable_permissioned_handle(
        master: address, key: DelegationKey, count_rate: bool
    ): &StorablePermissionedHandle {
    ...
    fun test_basics(account: signer, account_copy: signer) acquires RegisteredDelegations {
        ...
        let auth_data =
            auth_data::create_auth_data(vector[1, 2, 3], bcs::to_bytes(&sig_bundle));
    ...
    fun test_rate_limit(
        account: signer, account_copy: signer, account_copy_2: signer
    ) acquires RegisteredDelegations {
        let aptos_framework = create_signer_for_test(@aptos_framework);
        timestamp::set_time_has_started_for_testing(&aptos_framework);
        let (sk, vpk) = generate_keys();
        ...
        add_permissioned_handle(
            &account,
            key,
            option::some(rate_limiter::initialize(1, 10)),
            60
        );
