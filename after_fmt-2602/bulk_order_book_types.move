...
module aptos_experimental::bulk_order_book_types {
    ...
    public(friend) fun new_bulk_order<M: store + copy + drop>(
        ...
        let bid_price_crossing_idx =
            discard_price_crossing_levels(&bid_prices, best_ask_price, true);
        let ask_price_crossing_idx =
            discard_price_crossing_levels(&ask_prices, best_bid_price, false);

        // Extract cancelled levels (the ones that were discarded)
        let (
            cancelled_bid_prices,
            cancelled_bid_sizes,
            post_only_bid_prices,
            post_only_bid_sizes
        ) =
    ...
    public fun get_sequence_number_from_order_request<M: store + copy + drop>(
        ...
        let BulkOrderRequest::V1 { order_sequence_number: sequence_number,.. } =
            order_req;
        *sequence_number
    }
    ...
    public fun new_bulk_order_place_response_rejection<M: store + copy + drop>(
        ...
        BulkOrderPlaceResponse::Rejection { reason: rejection_reason }
    }

    public fun is_success<M: store + copy + drop>(
        response: &BulkOrderPlaceResponse<M>
    ): bool {
        if (response is BulkOrderPlaceResponse::Success) { true }
        else { false }
    }
    ...
    public fun is_rejection<M: store + copy + drop>(
        ...
        if (response is BulkOrderPlaceResponse::Rejection) { true }
        else { false }
    }
    ...
    public fun destroy_bulk_order_place_success_response<M: store + copy + drop>(
        ...
        let BulkOrderPlaceResponse::Success {
            order,
            cancelled_bid_prices,
            cancelled_bid_sizes,
            cancelled_ask_prices,
    ...
    fun validate_no_price_crossing(
        bid_prices: &vector<u64>, ask_prices: &vector<u64>
    ): bool {
    ...
    public(friend) fun reinsert_order<M: store + copy + drop>(
        self: &mut BulkOrder<M>, other: &OrderMatchDetails<M>
    ) {
        // Reinsert the order into the bulk order
        let (prices, sizes) =
            if (other.is_bid_from_match_details()) {
    ...
    public(friend) fun match_order_and_get_next<M: store + copy + drop>(
        self: &mut BulkOrder<M>, is_bid: bool, matched_size: u64
