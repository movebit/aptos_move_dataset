spec aptos_framework::aptos_governance {
    ...
    spec update_governance_config(
        ...
        aborts_if !features::spec_is_enabled(features::MODULE_EVENT_MIGRATION)
            && !exists<GovernanceEvents>(@aptos_framework);
    ...
    spec schema CreateProposalAbortsIf {
        ...
        aborts_if option::is_some(maybe_supply)
    ...
    spec schema VoteAbortIf {
        ...
        let can_be_resolved_early_0 = option::is_some(
            proposal.early_resolution_vote_threshold
        ) && (
        ...
        let can_be_resolved_early_1 = option::is_some(
            proposal.early_resolution_vote_threshold
        ) && (
        ...
        let can_be_resolved_early_2 = option::is_some(
            proposal.early_resolution_vote_threshold
        ) && (
        ...
        let can_be_resolved_early_3 = option::is_some(
            proposal.early_resolution_vote_threshold
        ) && (
            proposal.yes_votes >= early_resolution_threshold
        ...
        let proposal_state_successed_3 = is_voting_closed_3
            ...
            && proposal.yes_votes + new_proposal_no_votes_1
                >= proposal.min_vote_threshold;
        // post state
        let post can_be_resolved_early = option::is_some(
            proposal.early_resolution_vote_threshold
        ) && (
    ...
    spec schema AddApprovedScriptHash {
        ...
        aborts_if (
            timestamp::now_seconds() > proposal.expiration_secs
                || option::is_some(proposal.early_resolution_vote_threshold)
                    && (
                        proposal.yes_votes >= early_resolution_threshold
                            || proposal.no_votes >= early_resolution_threshold
                    )
    ...
    spec assert_proposal_expiration(stake_pool: address, proposal_id: u64) {
        ...
        let proposal_expiration = voting::spec_get_proposal_expiration_secs<
            GovernanceProposal>(@aptos_framework, proposal_id);
