...
module aptos_experimental::single_order_book {
        ...
        let order_with_state =
            if (remaining_size == 0) {
                ...
                self.orders.modify_and_return(
                    &order_id,
                    |order_with_state| {
                        aptos_experimental::single_order_types::set_remaining_size(
        ...
        let order_opt =
            self.orders.modify_if_present_and_return(
                ...
                |order_with_state| {
                    assert!(
                        order_creator
                            == order_with_state.get_order_from_state().get_account(),
                        EORDER_CREATOR_MISMATCH
                    );
                    // TODO should we be asserting that remaining size is greater than 0?
                    aptos_experimental::single_order_types::decrease_remaining_size(
    ...
    public(friend) fun update_order_and_get_matches<M: store + copy + drop>(
        ...
        self.place_order_and_get_matches(
            price_time_idx, ascending_id_generator, order_req
        )
    }
    ...
    #[test_only]
    public fun set_up_test_with_id()
        : (SingleOrderBook<u64>, PriceTimeIndex, AscendingIdGenerator) {
    ...
    fun test_price_move_down_condition() {
        ...
        assert!(
            order_book.trigger_pending_orders(
                &mut price_time_idx, &mut ascending_id_generator, 100
            ).is_empty()
        );
        ...
        let match_results =
            order_book.trigger_pending_orders(
                &mut price_time_idx, &mut ascending_id_generator, 90
            );
    ...
    fun test_price_move_up_condition() {
        ...
        let match_results =
            order_book.trigger_pending_orders(
                &mut price_time_idx, &mut ascending_id_generator, 110
            );
        assert!(match_results.length() == 1);
        ...
        let match_results =
            order_book.trigger_pending_orders(
                &mut price_time_idx, &mut ascending_id_generator, 120
            );
