...
module staking::commission_tests {
    ...
    fun test_distribution_with_no_debt(manager: &signer) {
        ...
        assert!(commission::commission_owed() == 0);
        assert!(
            coin::balance<AptosCoin>(signer::address_of(manager))
                == expected_apt_amount
        );
    ...
    fun test_distribution_with_debt(manager: &signer) {
        ...
        assert!(
            coin::balance<AptosCoin>(signer::address_of(manager)) == 0
        );
        assert!(
            coin::balance<AptosCoin>(OPERATOR) == expected_apt_amount / 2
        );
    }
    ...
    inline fun usd_to_apt(amount_usd: u64): u64 {
        (
            math128::mul_div(
                (amount_usd as u128) * math128::pow(10, 8),
                math128::pow(10, 8),
                APT_PRICE
            ) as u64
        )
