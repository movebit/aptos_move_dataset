module 0x42::M {
    ...
    inline fun must_find_position_mut(address: address, x: u64): &mut S {
        if (exists<IsolatedPosition>(address)) {
            ...
            let (is_found, index) = vector::find(positions, |position| &position.x
                == &x);
            if (is_found) {
    ...
    inline fun must_find_position(address: address, x: u64): &S {
        if (exists<IsolatedPosition>(address)) {
