...
module post_mint_reveal_nft::minting {
    ...
    public entry fun set_treasury(
        admin: &signer, new_treasury_address: address
    ) acquires NFTMintConfig {
        assert!(
            signer::address_of(admin) == @post_mint_reveal_nft,
            error::permission_denied(ENOT_AUTHORIZED)
        );
        assert!(
            account::exists_at(new_treasury_address),
    ...
    public entry fun set_collection_config_and_create_collection(
        ...
        assert!(
            vector::length(&destination_collection_mutate_config) == 3
                && vector::length(&token_mutate_config) == 5,
        ...
        if (exists<CollectionConfig>(@post_mint_reveal_nft)) {
            ...
            collection_config.token_mutate_config = create_token_mutability_config(
                &token_mutate_config
            );
            ...
            move_to(
                ...
                CollectionConfig {
                    ...
                    token_description,
                    token_maximum,
                    token_mutate_config: create_token_mutability_config(
                        &token_mutate_config
                    ),
                    royalty_points_den,
                    royalty_points_num,
    ...
    public entry fun set_public_minting_and_reveal_config(
        ...
        if (exists<RevealConfig>(@post_mint_reveal_nft)) {
            ...
            move_to(admin, RevealConfig { reveal_time });
        };
    }
    ...
    public entry fun mint_source_certificate(
        nft_claimer: &signer, amount: u64
    ) acquires NFTMintConfig, PublicMintConfig, SourceToken, CollectionConfig {
        assert!(
            exists<CollectionConfig>(@post_mint_reveal_nft)
                && exists<PublicMintConfig>(@post_mint_reveal_nft),
            error::permission_denied(ECONFIG_NOT_INITIALIZED)
        );
        ...
        let is_public_minting_time =
            now >= public_mint_config.public_minting_start_time
                && now < public_mint_config.public_minting_end_time;
        assert!(
            ...
            if (collection_config.public_mint_limit != 0) {
                ...
                let limit =
                    bucket_table::borrow_mut(
                        &mut public_mint_config.public_minting_addresses, claimer_addr
                    );
                assert!(
                    amount <= *limit,
                    error::invalid_argument(EAMOUNT_EXCEEDS_MINTS_ALLOWED)
                );
                *limit = *limit - amount;
            };
        };
    ...
    entry fun exchange(
        ...
        assert!(
            exists<CollectionConfig>(@post_mint_reveal_nft)
                && exists<RevealConfig>(@post_mint_reveal_nft),
            error::permission_denied(ECONFIG_NOT_INITIALIZED)
        );
        ...
        let source_collection_name =
            borrow_global<SourceToken>(@post_mint_reveal_nft).source_collection_name;
        let token_id =
        ...
        assert!(
            big_vector::length(&collection_config.tokens) > 0,
        ...
        string::append(
            &mut token_name,
            string::sub_string(
                &source_token_name, index, string::length(&source_token_name)
            )
        );
    ...
    public fun acquire_resource_signer(admin: &signer): signer acquires NFTMintConfig {
        assert!(
            signer::address_of(admin) == @post_mint_reveal_nft,
            error::permission_denied(ENOT_AUTHORIZED)
        );
    ...
    public entry fun test_happy_path(
        ...
        assert!(
            token::balance_of(
                signer::address_of(&public_nft_claimer), exchanged_token_id3
            ) == 1,
            5
        );
